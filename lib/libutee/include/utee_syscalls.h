...
psa_status_t _utee_cryp_psa_obj_generate_key(const struct psa_key_attributes_s *attributes, mbedtls_svc_key_id_t *key);

mbedtls_psa_cipher_operation* _utee_cryp_psa_cipher_operation_init(void);

psa_status_t _utee_cryp_psa_cipher_encrypt_setup(mbedtls_psa_cipher_operation *operation,
												 mbedtls_svc_key_id_t key,
												 psa_algorithm_t alg);

psa_status_t _utee_cryp_psa_cipher_decrypt_setup(mbedtls_psa_cipher_operation *operation,
												 mbedtls_svc_key_id_t key,
												 psa_algorithm_t alg);

psa_status_t _utee_cryp_psa_cipher_generate_iv(mbedtls_psa_cipher_operation *operation,
											   uint8_t *iv,
											   size_t iv_size,
											   size_t *iv_length);

psa_status_t _utee_cryp_psa_cipher_set_iv(mbedtls_psa_cipher_operation *operation,
										  const uint8_t *iv,
										  size_t iv_length);

psa_status_t _utee_cryp_psa_cipher_update(mbedtls_psa_cipher_operation *operation,
										  const uint8_t *input,
										  size_t input_length,
										  uint8_t *output,
										  size_t output_size,
...
psa_status_t _utee_cryp_psa_cipher_finish(mbedtls_psa_cipher_operation *operation,
										  uint8_t *output,
										  size_t output_size,
										  size_t *output_length);

psa_status_t _utee_cryp_psa_cipher_abort(mbedtls_psa_cipher_operation *operation);
...
psa_status_t _utee_psa_aead_encrypt(mbedtls_svc_key_id_t key,
                                    psa_algorithm_t alg,
                                    const syscallStruct syscalls_struct,
...
psa_status_t _utee_psa_aead_decrypt(mbedtls_svc_key_id_t key,
                                    psa_algorithm_t alg,
                                   const syscallStruct syscalls_struct,
...
static  psa_aead_operation* _utee_psa_aead_operation_init(void);

psa_status_t _utee_psa_aead_encrypt_setup( psa_aead_operation *operation,
                                          mbedtls_svc_key_id_t key,
                                          psa_algorithm_t alg);

psa_status_t _utee_psa_aead_decrypt_setup( psa_aead_operation *operation,
                                          mbedtls_svc_key_id_t key,
                                          psa_algorithm_t alg);

psa_status_t _utee_psa_aead_generate_nonce( psa_aead_operation *operation,
                                           uint8_t *nonce,
                                           size_t nonce_size,
                                           size_t *nonce_length);

psa_status_t _utee_psa_aead_set_nonce( psa_aead_operation *operation,
                                      const uint8_t *nonce,
                                      size_t nonce_length);

psa_status_t _utee_psa_aead_set_lengths( psa_aead_operation *operation,
                                        size_t ad_length,
                                        size_t plaintext_length);

psa_status_t _utee_psa_aead_update_ad( psa_aead_operation *operation,
                                      const uint8_t *input,
                                      size_t input_length);

psa_status_t _utee_psa_aead_update( psa_aead_operation *operation,
                                   const uint8_t *input,
                                   size_t input_length,
                                   uint8_t *output,
                                   size_t output_size,
...
psa_status_t _utee_psa_aead_finish( psa_aead_operation *operation,
                                   uint8_t *ciphertext,
                                   size_t ciphertext_size,
                                   size_t *ciphertext_length,
                                   uint8_t *tag,
...
psa_status_t _utee_psa_aead_verify( psa_aead_operation *operation,
                                   uint8_t *plaintext,
                                   size_t plaintext_size,
                                   size_t *plaintext_length,
                                   const uint8_t *tag,
...
psa_status_t _utee_psa_aead_abort( psa_aead_operation *operation);

psa_status_t _utee_psa_sign_message(mbedtls_svc_key_id_t key,
                                    psa_algorithm_t alg,
                                    const uint8_t *input,
                                    size_t input_length,
                                    uint8_t *signature,
...
psa_status_t _utee_psa_verify_message(mbedtls_svc_key_id_t key,
                                      psa_algorithm_t alg,
                                      const uint8_t *input,
                                      size_t input_length,
...
psa_status_t _utee_psa_sign_hash(mbedtls_svc_key_id_t key,
                                 psa_algorithm_t alg,
                                 const uint8_t *hash,
                                 size_t hash_length,
                                 uint8_t *signature,
...
psa_status_t _utee_psa_verify_hash(mbedtls_svc_key_id_t key,
                                   psa_algorithm_t alg,
                                   const uint8_t *hash,
                                   size_t hash_length,
                                   const uint8_t *signature,
...
psa_status_t _utee_psa_asymmetric_encrypt(mbedtls_svc_key_id_t key,
                                          psa_algorithm_t alg,
                                          const uint8_t *input,
                                          size_t input_length,
                                          const uint8_t *salt,
...
psa_status_t _utee_psa_asymmetric_decrypt(mbedtls_svc_key_id_t key,
                                          psa_algorithm_t alg,
                                          const uint8_t *input,
                                          size_t input_length,
                                          const uint8_t *salt,
