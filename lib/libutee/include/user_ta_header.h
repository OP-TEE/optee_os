...
#define TA_FLAG_DEVICE_ENUM_SUPP	(1 << 10) /* with tee-supplicant */
	...
	/*
	 * The flag is to determine if TA get panic when an unsupported
	 * function ID is called. If this flag is set, it returns an error
	 * -- TEE_ERROR_NOT_IMPLEMENTED instead of calling TEE_Panic().
	 *
	 * This is to ensure backward compatible when add new function to
	 * user TA entry.
	 */
