...
def get_args():
        ...
        parser = argparse.ArgumentParser(description='Converts a Trusted '
                                         'Application (TA) ELF file into a C '
                                         'source file, ready for inclusion in '
                                         'the TEE binary as an "early TA".')

        parser.add_argument('--out', required=True,
                            help='Name of the output C file')

        parser.add_argument('--ta', required=True,
                            help='Path to the TA binary. File name has to '
                            'be: <uuid>.* such as: '
        ...
        parser.add_argument('--compress', dest="compress",
                            action="store_true",
                            help='Compress the TA using the DEFLATE algorithm')
