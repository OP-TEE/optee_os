...
def main():
    ...
    for section in config.sections():
        if 'allowed' not in config[section]:
            continue
        allowed_c = []
    ...
    outf.write('#include <load_policy.h>\n#include <tee_api_types.h>\n\n')
    ...
    outf.write('\nconst struct load_policy load_policies[] = {\n')
    for section in config.sections():
        if section == '*':
            uuid_c = 'ANY_UUID'
        else:
        ...
        varname = 'NULL'
        if 'allowed' in config[section]:
            allowed = config[section]['allowed']
            varname = policies[allowed]['varname']
        outf.write(f'\t{{ {uuid_c}, ')
        if 'tag' in config[section]:
            tag = bytearray.fromhex(config[section]['tag'])
            outf.write(tag_to_c(tag))
            outf.write(f',  {len(tag)}, ')
        else:
            outf.write('NULL, 0, ')
        outf.write(f'{varname} }},\n')
    outf.write('\t{ }\n};\n')
    outf.close()
