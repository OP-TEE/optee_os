				     ...
				     size_t *roots_der_len)
{
	 ...
	 */
	if (rc) {
		uint32_t ignorable = MBEDTLS_X509_BADCERT_EXPIRED |
				     MBEDTLS_X509_BADCERT_FUTURE;

		if (!flags || (flags & ~ignorable)) {
			EMSG("PAS auth: cert chain verify failed (%#"PRIx32")",
			     flags);
			goto out;
		}
	}
				      ...
				      size_t hash_len)
{
	...
	if (!data || !expected || !hash_len || hash_len > digest_buf_len)
		return TEE_ERROR_BAD_PARAMETERS;

	res = digest(hash_algo, data, data_len, digest_buf, &digest_buf_len);
	if (res != TEE_SUCCESS)
		goto out;

	if (digest_buf_len != hash_len) {
		res = TEE_ERROR_SECURITY;
		goto out;
	}

	if (consttime_memcmp(digest_buf, expected, hash_len) != 0)
		res = TEE_ERROR_SECURITY;
	else
		res = TEE_SUCCESS;
out:
