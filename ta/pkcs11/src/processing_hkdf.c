...
/* RFC 5869 2.2: PRK = HMAC-Hash(salt, IKM); empty salt uses HashLen zeros */
...
parse_hkdf_params(struct pkcs11_attribute_head *proc_params,
		  uint32_t *extract, uint32_t *expand, uint32_t *prf_hash,
		  uint32_t *salt_type, void **salt, uint32_t *salt_len,
		  uint32_t *salt_handle, void **info, uint32_t *info_len)
{
				  ...
				  struct obj_attrs **head)
{
	...
	if (extract == PKCS11_TRUE) {
		...
		case PKCS11_CKF_HKDF_SALT_KEY:
			...
			attrs = salt_obj->attributes;
			rc = check_access_attrs_against_token(session, attrs);
			if (rc)
				goto out;
			rc = get_attribute_ptr(attrs, PKCS11_CKA_VALUE,
	...
	if (rc == PKCS11_RV_NOT_FOUND) {
		...
		/* When bExpand is false, default length is the hash length */
