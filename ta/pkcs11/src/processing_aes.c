 ...
 *
 * As per PKCS#11, CCM/GCM decryption shall not reveal the data until the
 ...
 * data are allocated during AE update processing and copied into client's
 * buffer at AE finalization.
 *
 * As per PKCS#11, CCM/GCM decrypt expect the tag/MAC data to be provided
 * inside the input data for C_DecryptUpdate() and friends. But the
 * C_DecryptFinal() API does not provide input data reference hence we do not
 * know which is the last call to C_DecryptUpdate() where last bytes are not
 * ciphered data but the requested tag/MAC byte. To handle this, the TA saves
	...
	if (ctx->pending_size &&
		...
		if (ct_size) {
			...
			if (!ct_size) {
				TEE_Free(ct);
				ct = NULL;
				DMSG_RAW("We expected some data!\n\n");
			}
		}
				    ...
				    void *out, size_t *out_size)
{
	...
	if (res == TEE_ERROR_SHORT_BUFFER) {
		...
		if (!data_size) {
			TEE_Free(data_ptr);
			data_ptr = NULL;
			DMSG("Is this expected from the Core API?");
		}
	}
