...
TEE_Result tee_ta_dump_stats(void *buff, uint32_t *buff_size)
{
	TEE_Result res = TEE_SUCCESS;
	struct tee_ta_param param;
	struct tee_ta_dump_stats *dump_stats = (struct tee_ta_dump_stats *) buff;
	struct tee_ta_session *sess = NULL;
	struct tee_ta_session_head *open_sessions = NULL;
	struct tee_ta_ctx *ctx = NULL;
	struct user_ta_ctx *utc = NULL;
	...
	TAILQ_FOREACH(ctx, &tee_ctxes, link) {
		...
		memcpy(&dump_stats->uuid, &ctx->ts_ctx.uuid, sizeof(ctx->ts_ctx.uuid));
		...
		TAILQ_FOREACH(sess, open_sessions, link) {
			if (sess->ts_sess.ctx == &ctx->ts_ctx && count < 16) {
				dump_stats->sess_stats[count].id = sess->id;
				dump_stats->sess_stats[count].clnt_id = sess->clnt_id.login;
				dump_stats->sess_stats[count].cancel = sess->cancel;
				dump_stats->sess_stats[count].cancel_mask = sess->cancel_mask;
				dump_stats->sess_stats[count].ref_count = sess->ref_count;
				dump_stats->sess_stats[count].lock_thread = sess->lock_thread;
				dump_stats->sess_stats[count].unlink = sess->unlink;
				if (is_user_ta_ctx(&ctx->ts_ctx)) {
					/*heap info*/
					...
					param.types = TEE_PARAM_TYPES(TEE_PARAM_TYPE_VALUE_OUTPUT,
										...
										TEE_PARAM_TYPE_NONE,
										TEE_PARAM_TYPE_NONE);
					tee_ta_dump_memstats(sess, TEE_TIMEOUT_INFINITE, &param);
					dump_stats->sess_stats[count].stats.allocated = param.u[0].val.a;
					dump_stats->sess_stats[count].stats.max_allocated = param.u[0].val.b;
					dump_stats->sess_stats[count].stats.size = param.u[1].val.a;
				}
				count ++;
			}
		}
