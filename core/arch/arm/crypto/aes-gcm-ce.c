				  ...
				  const void *key, size_t key_len)
{
	...
	crypto_aes_gcm_encrypt_block(ctx, ctx->ctr, ctx->hash_subkey);
...
static void get_dg(uint64_t dg[2], struct crypto_aes_gcm_ctx *ctx)
{
	dg[1] = get_be64(ctx->hash_state);
	dg[0] = get_be64(ctx->hash_state + 8);
}

static void put_dg(struct crypto_aes_gcm_ctx *ctx, uint64_t dg[2])
{
