...
static TEE_Result ti_sci_mailbox_clear_thread(void)
{
	uint32_t try_count = 0;

	try_count = io_read32(mailbox_rx_base + TI_MAILBOX_MSG_STATUS);
	while (io_read32(mailbox_rx_base + TI_MAILBOX_MSG_STATUS) != 0U) {
		io_read32(mailbox_rx_base + TI_MAILBOX_MSG);
		if (try_count == 0U) {
			EMSG("Mailbox RX status polling timed out");
			return TEE_ERROR_TIMEOUT;
		}
...
TEE_Result ti_sci_transport_send(const struct ti_sci_msg *msg)
{
	...
	FMSG("Clearing the thread");
	ret = ti_sci_mailbox_clear_thread();
	if (ret) {
		EMSG("Thread verification failed. ret = %d", ret);
		return ret;
	}
...
TEE_Result ti_sci_transport_init(void)
{
	mailbox_rx_base = core_mmu_get_va(TI_MAILBOX_RX_BASE,
					       MEM_AREA_IO_SEC, TI_MAILBOX_DEFAULT_SIZE);
	if (!mailbox_rx_base)
