...
TEE_Result sec_elf_get_size(const uint8_t *data, size_t capacity, size_t *size)
{
	...
	*size = 0;
	if (capacity < sizeof(*hdr)) {
		EMSG("sec.elf too small: %zu bytes", capacity);
		return TEE_ERROR_BAD_FORMAT;
	}

	hdr = (const struct secdat_hdr *)data;
	if (hdr->magic1 != SECDAT_MAGIC1 || hdr->magic2 != SECDAT_MAGIC2) {
		EMSG("Invalid sec.elf magic: %#"PRIx32"/%#"PRIx32,
		     hdr->magic1, hdr->magic2);
		return TEE_ERROR_BAD_FORMAT;
	}

	if (hdr->revision != SECDAT_VERSION_2) {
		EMSG("Unsupported sec.elf version: %#"PRIx32, hdr->revision);
		return TEE_ERROR_BAD_FORMAT;
	}

	if (hdr->seg_num > SECDAT_MAX_SUPPORTED_SEGMENT) {
		EMSG("Too many segments: %"PRIu32, hdr->seg_num);
		return TEE_ERROR_BAD_FORMAT;
	}

	if (hdr->size < sizeof(struct secdat_footer) ||
	    MUL_OVERFLOW((size_t)hdr->seg_num, sizeof(struct segment_hdr),
			 &table_size) ||
	    ADD_OVERFLOW(sizeof(*hdr), table_size, &total) ||
	    ADD_OVERFLOW(total, (size_t)hdr->size, &total) ||
	    total > capacity) {
		EMSG("sec.elf size mismatch: header %"PRIu32" capacity %zu",
		     hdr->size, capacity);
		return TEE_ERROR_BAD_FORMAT;
	}
				     ...
				     bool check_shk_count)
{
	...
	if (size < sizeof(*hdr)) {
		EMSG("Fuse list too small: %zu bytes", size);
		return TEE_ERROR_BAD_FORMAT;
	}
	...
	if (hdr->revision != SECDAT_FUSE_LIST_REVISION || !hdr->fuse_count ||
		...
		EMSG("Invalid fuse list: rev %"PRIu32" count %"PRIu32
		     " size %"PRIu32, hdr->revision, hdr->fuse_count,
		     hdr->size);
		return TEE_ERROR_BAD_FORMAT;
	}
	...
	for (n = 0; n < hdr->fuse_count; n++) {
		if (entries[n].region >= FUSEPROV_REGION_MAX ||
			...
			EMSG("Invalid fuse entry %"PRIu32": region %"PRIu32
			     " op %"PRIu32, n, entries[n].region,
			     entries[n].operation);
			return TEE_ERROR_BAD_FORMAT;
		}
		...
		if (check_shk_count &&
			...
			EMSG("Too many SHK entries (max %u)",
			     SECDAT_MAX_SHK_ROWS);
			return TEE_ERROR_BAD_FORMAT;
		}
	}
	...
	*segments = NULL;
	...
	for (n = 0; n < header->seg_num; n++) {
		...
		if (table[n].offset < payload_start ||
			...
			EMSG("Invalid segment %"PRIu32" offset %"PRIu32, n,
			     table[n].offset);
			return TEE_ERROR_BAD_FORMAT;
		}

		if (!efuse && table[n].type != SECDAT_SEGMENT_TYPE_ENCKEY)
			continue;

		res = validate_fuse_list(data + table[n].offset,
					 end - table[n].offset, efuse);
		if (res)
			return res;
	}
