...
{
	const struct secdat_hdr *header;
	...
	if (header->magic1 != SECDAT_MAGIC1) {
		EMSG("Invalid magic1: 0x%08x", header->magic1);
		return TEE_ERROR_BAD_FORMAT;
	}

	if (header->magic2 != SECDAT_MAGIC2) {
		EMSG("Invalid magic2: 0x%08x", header->magic2);
		return TEE_ERROR_BAD_FORMAT;
	}

	if (header->revision != SECDAT_VERSION_2) {
		EMSG("Unsupported version: 0x%08x", header->revision);
		return TEE_ERROR_BAD_FORMAT;
	}

	if (header->size + sizeof(*header) + sizeof(struct secdat_footer) !=
	    size) {
		EMSG("Size mismatch: header=%u, file=%zu", header->size, size);
		return TEE_ERROR_BAD_FORMAT;
	}

	if (header->seg_num > SECDAT_MAX_SUPPORTED_SEGMENT) {
		EMSG("Too many segments: %u", header->seg_num);
		return TEE_ERROR_BAD_FORMAT;
	}
...
{
	TEE_Result res;
	uint8_t calc_hash[TEE_SHA256_HASH_SIZE];
	...
	if (res != TEE_SUCCESS) {
		EMSG("Failed to allocate hash context: 0x%x", res);
		goto out;
	}

	res = crypto_hash_init(ctx);
	if (res != TEE_SUCCESS) {
		EMSG("Failed to initialize hash: 0x%x", res);
		goto out;
	}

	res = crypto_hash_update(ctx, data, hash_bytes);
	if (res != TEE_SUCCESS) {
		EMSG("Failed to update hash: 0x%x", res);
		goto out;
	}

	res = crypto_hash_final(ctx, calc_hash, TEE_SHA256_HASH_SIZE);
	if (res != TEE_SUCCESS) {
		EMSG("Failed to finalize hash: 0x%x", res);
		goto out;
	}
