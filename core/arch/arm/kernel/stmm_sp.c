...
{
	struct ffa_boot_info_header_1_1 *ffa_boot_info_hdr = NULL;
	struct ffa_boot_info_1_1 *ffa_boot_info_desc = NULL;
	struct efi_hob_handoff_info_table *hob_table = NULL;
	...
	ffa_boot_info_hdr = (void *)stmm_sec_buf_addr;

	ffa_boot_info_hdr->signature = FFA_BOOT_INFO_SIGNATURE;
	ffa_boot_info_hdr->version = FFA_VERSION_1_2;
	ffa_boot_info_hdr->desc_size =
		sizeof(struct ffa_boot_info_1_1);
	ffa_boot_info_hdr->desc_count = 1;

	ffa_boot_info_hdr->desc_offset =
		sizeof(struct ffa_boot_info_header_1_1);

	ffa_boot_info_hdr->reserved = 0;

	ffa_boot_info_hdr->blob_size =
		ffa_boot_info_hdr->desc_size * ffa_boot_info_hdr->desc_count +
		ffa_boot_info_hdr->desc_offset;

	ffa_boot_info_desc = (void *)
		(stmm_sec_buf_addr + ffa_boot_info_hdr->desc_offset);

	memset(ffa_boot_info_desc->name, 0, FFA_BOOT_INFO_NAME_LEN);
	...
	ffa_boot_info_desc->contents = (vaddr_t)hob_table;
	...
	init_stmm_regs(spc, (unsigned long)ffa_boot_info_hdr, 0, 0, 0,
		       stmm_image_addr);
