...
#define __FFA_SUCCESS	FFA_SUCCESS_64
...
#define __FFA_SUCCESS	FFA_SUCCESS_32
			 ...
			 uint32_t sp_size, uint32_t *hob_table_size)
{
	...
	unsigned int uncompressed_size_roundup;
	struct efi_mmram_descriptor *mmram_desc_data;
	struct efi_mmram_hob_descriptor_block *mmram_resv_data;
	uint16_t mmram_resv_data_size;
	TEE_Result ret;
	uint32_t hob_table_offset;
	void *guid_hob_data;
	...
	hob_table = create_hob_list(sp_addr, sp_size,
	...
	ret = create_fv_hob(hob_table, sp_addr, uncompressed_size_roundup);
	if (ret) {
		EMSG("Failed to create fv hob.\n");
		return NULL;
	}

	ret = create_resource_desc_hob(hob_table,
				       EFI_RESOURCE_SYSTEM_MEMORY,
				       EFI_RESOURCE_ATTR_PRESENT |
				       EFI_RESOURCE_ATTR_TESTED |
				       EFI_RESOURCE_ATTR_UNCACHEABLE |
				       EFI_RESOURCE_ATTR_WR_COMBINEABLE |
				       EFI_RESOURCE_ATTR_WR_THROUGH_CACHEABLE |
				       EFI_RESOURCE_ATTR_WR_BACK_CACHEABLE,
				       sp_addr, sp_size);
	if (ret) {
		EMSG("Failed to create resource descriptor hob\n");
		return NULL;
	}
...
{
	struct ffa_boot_info_header_1_1 *ffa_boot_info_header;
	struct ffa_boot_info_1_1 *ffa_boot_info_desc;
	struct efi_hob_handoff_info_table *hob_table;
	...
	ffa_boot_info_header =
		(struct ffa_boot_info_header_1_1 *)stmm_sec_buf_addr;

	ffa_boot_info_header->signature = FFA_BOOT_INFO_SIGNATURE;
	ffa_boot_info_header->version = FFA_VERSION_1_2;
	ffa_boot_info_header->desc_size =
		sizeof(struct ffa_boot_info_1_1);
	ffa_boot_info_header->desc_count = 1;

	ffa_boot_info_header->desc_offset =
		sizeof(struct ffa_boot_info_header_1_1);

	ffa_boot_info_header->reserved = 0x00;

	ffa_boot_info_header->blob_size =
		(ffa_boot_info_header->desc_offset +
			(ffa_boot_info_header->desc_size *
			 ffa_boot_info_header->desc_count));

	ffa_boot_info_desc = (struct ffa_boot_info_1_1 *)
		(stmm_sec_buf_addr + ffa_boot_info_header->desc_offset);

	memset(ffa_boot_info_desc->name, 0x00, FFA_BOOT_INFO_NAME_LEN);
	ffa_boot_info_desc->type = FFA_BOOT_INFO_TYPE_ID_HOB;
	ffa_boot_info_desc->flags = (FFA_BOOT_INFO_FLAG_NAME_FORMAT_UUID |
			(FFA_BOOT_INFO_FLAG_CONTENT_FORMAT_ADDR <<
			 FFA_BOOT_INFO_FLAG_CONTENT_FORMAT_SHIFT));
	ffa_boot_info_desc->size = hob_table_size;
	ffa_boot_info_desc->contents = (uint64_t)hob_table;
	...
	init_stmm_regs(spc, (unsigned long)ffa_boot_info_header,
		       0x00, 0x00, 0x00, stmm_image_addr);
...
static void spm_handle_get_mem_attr(struct thread_scall_regs *regs)
{
	...
	va = SVC_REGS_A1(regs);
	if (!va) {
		ffa_ret = FFA_INVALID_PARAMETERS;
		goto err;
	}
	...
	SVC_REGS_A0(regs) = __FFA_SUCCESS;
	SVC_REGS_A1(regs) = FFA_PARAM_MBZ;
