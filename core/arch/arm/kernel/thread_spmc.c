...
static void handle_blocking_call(struct thread_smc_1_2_regs *args)
{
	...
	case OPTEE_FFA_RELEASE_PROTMEM:
		spmc_set_args(args, direct_resp_fid, swap_src_dst(args->a1), 0,
			      handle_unregister_shm(args->a4, args->a5), 0, 0);
		break;
#endif
...
static void handle_mem_op(struct thread_smc_1_2_regs *args,
			  struct ffa_rxtx *rxtx)
{
...
TEE_Result thread_spmc_get_protmem_config(uint64_t use_case, void *buf,
					  size_t *buf_sz, size_t *min_mem_sz,
					  size_t *min_mem_align)
{
	struct ffa_mem_access_perm mem_acc_list[] = {
		{
			.endpoint_id = optee_core_lsp.sp_id,
			.perm = FFA_MEM_ACC_RW,
		},
	};

	switch (use_case) {
	case OPTEE_MSG_PROTMEM_SECURE_VIDEO_PLAY:
		break;
	case OPTEE_MSG_PROTMEM_TRUSTED_UI:
	case OPTEE_MSG_PROTMEM_SECURE_VIDEO_RECORD:
	default:
		return TEE_ERROR_BAD_PARAMETERS;
	}
	...
	*min_mem_align = SMALL_PAGE_SIZE;
	if (!buf || *buf_sz < sizeof(mem_acc_list)) {
		*buf_sz = sizeof(mem_acc_list);
		return TEE_ERROR_SHORT_BUFFER;
	}

	memcpy(buf, mem_acc_list, sizeof(mem_acc_list));
	*buf_sz = sizeof(mem_acc_list);
	return TEE_SUCCESS;
}
