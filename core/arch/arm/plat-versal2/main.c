				  ...
				  paddr_t *gicr_base)
{
	...
	*gicd_base = reg_pair_to_64(fdt32_to_cpu(reg[0]), fdt32_to_cpu(reg[1]));
	*gicr_base = reg_pair_to_64(fdt32_to_cpu(reg[4]), fdt32_to_cpu(reg[5]));
}
...
void boot_primary_init_intc(void)
{
	...
	if (fdt) {
		...
		if (gicd_base != GICD_BASE)
			IMSG("GICD base overridden by DTB: 0x%016"PRIxPA,
			     gicd_base);
		if (gicr_base != GICR_BASE)
			IMSG("GICR base overridden by DTB: 0x%016"PRIxPA,
			     gicr_base);
	} else {
		IMSG("External DT not found");
	}

	DMSG("GICD base: 0x%016"PRIxPA" GICR base: 0x%016"PRIxPA,
	     gicd_base, gicr_base);
