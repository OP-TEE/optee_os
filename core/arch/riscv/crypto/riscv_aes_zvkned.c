// SPDX-License-Identifier: BSD-2-Clause
/*
 * Copyright (c) 2026, RISCStar Solutions Limited
 */

/* AES ECB, CBC, and CTR acceleration using RISC-V vector crypto */

#include <assert.h>
#include <crypto/crypto_accel.h>
#include <kernel/thread.h>
#include <string.h>
#include <utee_defines.h>
#include <util.h>

#include "aes_riscv_zvkned.h"

#define AES_MAX_EXPANDED_KEY_SIZE 240

static const uint8_t aes_sbox[256] = {
	,
};

static void expand_enc_key(uint8_t *expanded, const uint8_t *key,
			   size_t key_len, size_t expanded_len)
{
	static const uint8_t rcon[] = {
		0x01, 0x02, 0x04, 0x08, 0x10,
		0x20, 0x40, 0x80, 0x1b, 0x36,
	};
	size_t generated = key_len;
	size_t rcon_idx = 0;
	uint8_t temp[4] = { };
	unsigned int i = 0;

	memcpy(expanded, key, key_len);
	while (generated < expanded_len) {
		memcpy(temp, expanded + generated - sizeof(temp), sizeof(temp));
		if (!(generated % key_len)) {
			uint8_t first = temp[0];

			temp[0] = aes_sbox[temp[1]] ^ rcon[rcon_idx++];
			temp[1] = aes_sbox[temp[2]];
			temp[2] = aes_sbox[temp[3]];
			temp[3] = aes_sbox[first];
		} else if (key_len == 32 && generated % key_len == 16) {
			for (i = 0; i < ARRAY_SIZE(temp); i++)
				temp[i] = aes_sbox[temp[i]];
		}
		for (i = 0; i < ARRAY_SIZE(temp) && generated < expanded_len;
		     i++, generated++)
			expanded[generated] = expanded[generated - key_len] ^
					      temp[i];
	}
}

TEE_Result crypto_accel_aes_expand_keys(const void *key, size_t key_len,
					void *enc_key, void *dec_key,
					size_t expanded_key_len,
					unsigned int *round_count)
{
	unsigned int rounds = 0;
	size_t required = 0;

	if (!key || !enc_key || !round_count)
		return TEE_ERROR_BAD_PARAMETERS;
	if (key_len != 16 && key_len != 24 && key_len != 32)
		return TEE_ERROR_BAD_PARAMETERS;
	if (!IS_ALIGNED(enc_key, sizeof(uint32_t)) ||
	    (dec_key && !IS_ALIGNED(dec_key, sizeof(uint32_t))))
		return TEE_ERROR_BAD_PARAMETERS;

	rounds = key_len / 4 + 6;
	required = (rounds + 1) * TEE_AES_BLOCK_SIZE;
	if (expanded_key_len < required || required > AES_MAX_EXPANDED_KEY_SIZE)
		return TEE_ERROR_BAD_PARAMETERS;

	memset(enc_key, 0, expanded_key_len);
	expand_enc_key(enc_key, key, key_len, required);
	if (dec_key) {
		memset(dec_key, 0, expanded_key_len);
		memcpy(dec_key, enc_key, required);
	}
	*round_count = rounds;

	return TEE_SUCCESS;
}

void crypto_accel_aes_ecb_enc(void *out, const void *in, const void *key,
			      unsigned int round_count,
			      unsigned int block_count)
{
	uint32_t state = 0;

	assert(out && in && key);
	assert(round_count == 10 || round_count == 12 || round_count == 14);
	if (!block_count)
		return;

	state = thread_kernel_enable_vfp();
	riscv_aes_ecb_encrypt(key, in, out,
				(size_t)block_count * TEE_AES_BLOCK_SIZE,
				round_count);
	thread_kernel_disable_vfp(state);
}

void crypto_accel_aes_ecb_dec(void *out, const void *in, const void *key,
			      unsigned int round_count,
			      unsigned int block_count)
{
	uint32_t state = 0;

	assert(out && in && key);
	assert(round_count == 10 || round_count == 12 || round_count == 14);
	if (!block_count)
		return;

	state = thread_kernel_enable_vfp();
	riscv_aes_ecb_decrypt(key, in, out,
				(size_t)block_count * TEE_AES_BLOCK_SIZE,
				round_count);
	thread_kernel_disable_vfp(state);
}

void crypto_accel_aes_cbc_enc(void *out, const void *in, const void *key,
			      unsigned int round_count,
			      unsigned int block_count, void *iv)
{
	uint32_t state = 0;

	assert(out && in && key && iv);
	assert(round_count == 10 || round_count == 12 || round_count == 14);
	if (!block_count)
		return;

	state = thread_kernel_enable_vfp();
	riscv_aes_cbc_encrypt(key, in, out,
				(size_t)block_count * TEE_AES_BLOCK_SIZE, iv,
				round_count);
	thread_kernel_disable_vfp(state);
}

void crypto_accel_aes_cbc_dec(void *out, const void *in, const void *key,
			      unsigned int round_count,
			      unsigned int block_count, void *iv)
{
	uint32_t state = 0;

	assert(out && in && key && iv);
	assert(round_count == 10 || round_count == 12 || round_count == 14);
	if (!block_count)
		return;

	state = thread_kernel_enable_vfp();
	riscv_aes_cbc_decrypt(key, in, out,
				(size_t)block_count * TEE_AES_BLOCK_SIZE, iv,
				round_count);
	thread_kernel_disable_vfp(state);
}

