...
static_assert(NOTIF_VALUE_DO_BOTTOM_HALF ==
	      OPTEE_ABI_ASYNC_NOTIF_VALUE_DO_BOTTOM_HALF);
			       ...
			       struct optee_msg_arg *rpc_arg)
{
	...
	if (tee_entry_std(arg, num_params))
		...
		rv = OPTEE_ABI_RETURN_OK;

	thread_rpc_shm_cache_clear(&thr->shm_cache);
	if (rpc_arg)
		thr->rpc_arg = NULL;

	if (rv == OPTEE_ABI_RETURN_OK &&
...
static uint32_t std_abi_entry(uint32_t a0, uint32_t a1, uint32_t a2,
			      uint32_t a3 __unused)
{
	const bool with_rpc_arg = true;

	switch (a0) {
	case OPTEE_ABI_CALL_WITH_ARG:
		return std_entry_with_parg(reg_pair_to_64(a1, a2),
					   !with_rpc_arg);
	case OPTEE_ABI_CALL_WITH_RPC_ARG:
		return std_entry_with_parg(reg_pair_to_64(a1, a2),
					   with_rpc_arg);
...
/*
 * Helper routine for the assembly function thread_std_abi_entry()
 *
 * Note: this function is weak just to make it possible to exclude it from
 * the unpaged area.
 */
uint32_t __weak __thread_std_abi_entry(uint32_t a0, uint32_t a1, uint32_t a2,
				       ...
				       uint32_t a5 __unused)
{
	...
	return std_abi_entry(a0, a1, a2, a3);
}
...
static struct mobj *thread_rpc_alloc_arg(size_t size)
{
	...
	uint64_t co;
	uint32_t rpc_args[THREAD_RPC_NUM_ARGS] = {
		OPTEE_ABI_RETURN_RPC_ALLOC, size
	};
	struct mobj *mobj = NULL;
...
uint32_t thread_rpc_cmd(uint32_t cmd, size_t num_params,
			struct thread_param *params)
{
	uint32_t rpc_args[THREAD_RPC_NUM_ARGS] = { OPTEE_ABI_RETURN_RPC_CMD };
	void *arg = NULL;
	uint64_t carg = 0;
	uint32_t ret = 0;
