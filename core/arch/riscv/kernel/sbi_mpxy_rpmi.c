...
struct sbi_mpxy_rpmi_context *sbi_mpxy_rpmi_ctx = NULL;
 ...
 * MPXY channels with the RPMI protocol, reading their attributes, and allocating
 * memory for handling notifications. The result is stored in a global context
 * (sbi_mpxy_rpmi_ctx). If probing fails, the context is set to NULL.
 */
...
int sbi_mpxy_rpmi_send_data(struct sbi_mpxy_rpmi_channel *channel, void *data)
{
	...
	case SBI_MPXY_RPMI_MSG_TYPE_SET_ATTRIBUTE:
		/* All RPMI Message Protocol Attributes of an SBI MPXY Channel are RO */
		ret = RPMI_ERR_NOTSUPP;
		break;
	case SBI_MPXY_RPMI_MSG_TYPE_SEND_WITH_RESPONSE:
		...
		ret = sbi_mpxy_send_message_with_response(
			channel->channel_id, message->data.service_id,
			message->data.request, message->data.request_len,
			message->data.response, message->data.max_response_len,
	...
	case SBI_MPXY_RPMI_MSG_TYPE_SEND_WITHOUT_RESPONSE:
		...
		ret = sbi_mpxy_send_message_without_response(
			channel->channel_id, message->data.service_id,
			message->data.request, message->data.request_len);
		break;
	default:
