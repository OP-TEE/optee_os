...
TEE_Result __weak crypto_aes_gcm_set_key(struct crypto_aes_gcm_ctx *ctx,
...
void __weak
crypto_aes_gcm_update_payload_block_aligned(struct crypto_aes_gcm_ctx *ctx,
					    TEE_OperationMode m,
					    const void *src, size_t num_blocks,
					    void *dst)
{
	size_t n;
	const uint8_t *s = src;
	...
	for (n = 0; n < num_blocks; n++) {
		if (m == TEE_MODE_ENCRYPT) {
			...
			crypto_aes_gcm_ghash_update(ctx, ctx->buf_cryp,
						    NULL, 0);
			...
			xor_block(ctx->buf_cryp, s);
			crypto_aes_gcm_ghash_update(ctx, s, NULL, 0);
			memcpy(d, ctx->buf_cryp, sizeof(ctx->buf_cryp));
...
void __weak crypto_aes_gcm_encrypt_block(struct crypto_aes_gcm_ctx *ctx,
