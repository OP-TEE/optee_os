...
static void ghash_update_pad_zero(struct crypto_aes_gcm_ctx *ctx,
				  const uint8_t *data, size_t len)
{
	...
	if (n) {
		if (ptr_is_block_aligned(data)) {
			crypto_aes_gcm_ghash_update(ctx, NULL, data, n);
		} else {
			size_t m;
			...
			for (m = 0; m < n; m++) {
				...
				memcpy(block, data + m * sizeof(block),
				       sizeof(block));
				crypto_aes_gcm_ghash_update(ctx, NULL,
						     (void *)block, 1);
			}
		}
	}
	...
	if (len - n * TEE_AES_BLOCK_SIZE) {
		...
		crypto_aes_gcm_ghash_update(ctx, block, NULL, 0);
	}
}

static void ghash_update_lengths(struct crypto_aes_gcm_ctx *ctx, uint32_t l1,
				 uint32_t l2)
{
			       ...
			       size_t nonce_len, size_t tag_len)
{
	...
	ctx->tag_len = tag_len;
	res = crypto_aes_gcm_set_key(ctx, key, key_len);
	if (res)
		return res;
	...
	crypto_aes_gcm_encrypt_block(ctx, ctx->ctr, ctx->buf_tag);
	crypto_aes_gcm_inc_ctr(ctx);
	if (mode == TEE_MODE_ENCRYPT) {
		 ...
		 */
		crypto_aes_gcm_encrypt_block(ctx, ctx->ctr, ctx->buf_cryp);
		crypto_aes_gcm_inc_ctr(ctx);
	}
				     ...
				     const void *data, size_t len)
{
	...
	while (l) {
		...
		crypto_aes_gcm_ghash_update(ctx, head, d, n);
					 ...
					 const void *src, size_t len, void *dst)
{
	...
	if (!ctx->payload_bytes && ctx->buf_pos) {
		...
		crypto_aes_gcm_ghash_update(ctx, ctx->buf_hash, NULL, 0);
		ctx->buf_pos = 0;
	}
...
static TEE_Result operation_final(struct crypto_aes_gcm_ctx *ctx,
				  TEE_OperationMode m, const uint8_t *src,
				  size_t len, uint8_t *dst)
{
	TEE_Result res;

	res = crypto_aes_gcm_update_payload(ctx, m, src, len, dst);
	if (res)
		return res;
