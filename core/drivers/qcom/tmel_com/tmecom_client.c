...
/*
 * Delay between re-checks of the mailbox when a wait iteration finds nothing
 * new and the channel's interrupt (if any) has not signalled since the last
 * pass.  Bounds the extra latency added by not spinning at full rate.
 */
#define TMECOM_MBOX_POLL_DELAY_US		20u
...
/*
 * Notification callback for IRQ-assisted wakeup.
 *
 * Runs in hard-interrupt context (see qcom_mbox_notify_cb_t's contract in
 * qcom_mbox_types.h): must not sleep, block, take a lock, or touch the
 * mailbox.  Just flags that tmecom_mbox_wait() can stop delaying between
 * checks.
 */
static void tmecom_mbox_notify(struct qcom_mbox_chan *chan __unused,
			       void *priv)
{
	struct tmecom_ctx *ctx = priv;

	ctx->irq_signalled = true;
}
 ...
 * internally, so every wait in this driver funnels through here.  On a
 * channel whose platform wired an interrupt, tmecom_mbox_notify() flags
 * @ctx->irq_signalled as soon as the transport has something to report, so
 * this loop only needs to re-check qcom_mbox_process() at full rate right
 * after that flag is set; otherwise it backs off by
 * TMECOM_MBOX_POLL_DELAY_US between passes instead of spinning at full
 * speed.  On a channel without an interrupt (itr_chip == NULL, so
 * irq_signalled is never set), this degrades to a fixed low-rate poll.
 */
					     ...
					     uint32_t timeout_us)
{
	...
	while (true) {
		...
		if (ctx->irq_signalled)
			ctx->irq_signalled = false;
		else
			udelay(TMECOM_MBOX_POLL_DELAY_US);
	}
}
...
TEE_Result tmecom_client_session_start(void)
{
	...
	if (!tmecom_ctx.chan) {
		if (qcom_mbox_request(TMECOM_MBOX_CHANNEL_NAME,
				      tmecom_mbox_notify, &tmecom_ctx,
				      &tmecom_ctx.chan)) {
		...
		if (qcom_mbox_enable_irq(tmecom_ctx.chan, true))
			DMSG("Mailbox IRQ unavailable; continuing on polling");
	}
			   ...
			   enum tmecom_response *tme_err)
{
	 ...
	 * callers.  tmecom_mbox_wait() below still blocks this function until
	 * RX_READY arrives (an interrupt only lets it back off between checks
	 * instead of spinning at full rate); it does not let this function
	 * return early.  So a non-blocking request still cannot return before
	 * its response arrives; what it still gets is delivery through
	 * @cb_api rather than through @generic_payload alone.  Callers that
	 * pass is_blocking = false therefore keep working unchanged.
	 */
