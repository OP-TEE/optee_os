...
/* Timeout waiting for AMC completion after dispatch. */
#define RPMH_CMD_COMPLETION_TIMEOUT_US	10000

/* Timeout waiting for AOP to finish booting. */
#define RPMH_AOP_INIT_TIMEOUT_US	100000
			      ...
			      struct rpmh_command_set *set)
{
	...
	if (!tcs_is_idle(RPMH_TCS_IDLE_TIMEOUT_US)) {
...
{
	uint64_t timer = timeout_init_us(RPMH_AOP_INIT_TIMEOUT_US);
