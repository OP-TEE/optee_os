...
{
	struct rpmh_vote *active = NULL;
	struct rpmh_vote *sleep = NULL;
	struct rpmh_vote *wake = NULL;
				...
				enum rsc_drv_id drv_id)
{
	...
	if (!drv) {
		const struct drv_config *config = NULL;
		...
		res = cmd_db_get_priority(rc->address,
					  ...
					  &priority);
		if (res != TEE_SUCCESS) {
			EMSG("Failed to query priority for addr 0x%"PRIx32,
			     rc->address);
			free(drv);
			return NULL;
		}
				  ...
				  bool explicit_cmd)
{
	...
	drv = rpmh_resource_command_get_voter(rc, drv_id);
