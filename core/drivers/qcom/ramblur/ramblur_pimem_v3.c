...
/* Some of these functions are used by macros in the auto generated header */
...
static void disable_sw_init_mode(int window)
{
	...
	out_dword_masked_ns(reg, mask, val, RAMBLUR_PIMEM_WINn_CTL_INI(window));
	readback_sync(reg, val, mask, RAMBLUR_PIMEM_WINn_CTL_SW_INIT_MODE_SHFT);
}
...
static void initialize_hardware_v3_0(int window)
{
	 ...
	 */
	mask = RAMBLUR_PIMEM_WINn_STATUS_HW_INIT_IN_PROGRESS_BMSK;
	reg = RAMBLUR_PIMEM_WINn_STATUS_ADDR(window);

	do {
		val = in_dword_masked(reg, mask) >>
		      RAMBLUR_PIMEM_WINn_STATUS_HW_INIT_IN_PROGRESS_SHFT;
...
static void set_size(int window, size_t size)
{
	...
	win_size = get_size(window);
	if (!win_size) {
		/* Initialize */
		out_dword(reg, (uint32_t)size);
		readback_sync(reg, size, RAMBLUR_PIMEM_WINn_SIZE_RMSK, 0);
...
{
	uint32_t regs[2] = {
		RAMBLUR_PIMEM_WINn_DATA_TXN_QSB_CTL_ADDR(window),
		RAMBLUR_PIMEM_WINn_OVERHEAD_TXN_QSB_CTL_ADDR(window),
	};
	uint32_t masks[2] = {
		RAMBLUR_PIMEM_WINn_DATA_TXN_QSB_CTL_AINNERCACHEABLE_BMSK,
		RAMBLUR_PIMEM_WINn_OVERHEAD_TXN_QSB_CTL_AINNERCACHEABLE_BMSK,
	};
	uint32_t inis[2] = {
		RAMBLUR_PIMEM_WINn_DATA_TXN_QSB_CTL_INI(window),
		RAMBLUR_PIMEM_WINn_OVERHEAD_TXN_QSB_CTL_INI(window),
	};
	uint32_t shift[2] = {
		RAMBLUR_PIMEM_WINn_DATA_TXN_QSB_CTL_AINNERCACHEABLE_SHFT,
		RAMBLUR_PIMEM_WINn_OVERHEAD_TXN_QSB_CTL_AINNERCACHEABLE_SHFT,
	};
