					     ...
					     uint8_t *k1, uint8_t *k2)
{
	...
	uint8_t const_rb[16] = { 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
		    ...
		    size_t input_size, uint8_t *output)
{
	...
	struct dcp_cipher_data data = { };
	struct dcp_align_buf x = { };
	struct dcp_align_buf y = { };
	struct dcp_align_buf last = { };
	unsigned int i = 0;
	uint8_t offset = 0;

	if (!input || !output || !init)
		return TEE_ERROR_BAD_PARAMETERS;
	...
	ret = dcp_calloc_align_buf(&x, DCP_AES128_BLOCK_SIZE);
	if (ret != TEE_SUCCESS) {
		ret = TEE_ERROR_OUT_OF_MEMORY;
		goto out;
	}

	ret = dcp_calloc_align_buf(&y, DCP_AES128_BLOCK_SIZE);
	if (ret != TEE_SUCCESS) {
		ret = TEE_ERROR_OUT_OF_MEMORY;
		goto out;
	}

	ret = dcp_calloc_align_buf(&last, DCP_AES128_BLOCK_SIZE);
	if (ret != TEE_SUCCESS) {
		ret = TEE_ERROR_OUT_OF_MEMORY;
		goto out;
	}
	...
	/* Get number of block */
	nb_blocks = (input_size + DCP_AES128_BLOCK_SIZE - 1) /
		    DCP_AES128_BLOCK_SIZE;
	if (nb_blocks == 0) {
		nb_blocks = 1;
		block_complete = false;
	} else {
		if (input_size % DCP_AES128_BLOCK_SIZE)
			block_complete = false;
		else
			block_complete = true;
	}
	...
	for (i = 0; i < nb_blocks - 1; i++) {
		dcp_xor(x.data, input + offset, y.data, DCP_AES128_BLOCK_SIZE);
		ret = dcp_cipher_do_update(&data, y.data, x.data,
					   DCP_AES128_BLOCK_SIZE);
		if (ret)
			goto out;
	...
	memcpy(last.data, input + offset, DCP_AES128_BLOCK_SIZE);

	if (block_complete) {
		dcp_xor(last.data, key1, last.data, DCP_AES128_BLOCK_SIZE);
	} else {
		dcp_cmac_padding(last.data, input_size % DCP_AES128_BLOCK_SIZE);
		dcp_xor(last.data, key2, last.data, DCP_AES128_BLOCK_SIZE);
	}

	dcp_xor(x.data, last.data, y.data, DCP_AES128_BLOCK_SIZE);
	ret = dcp_cipher_do_update(&data, y.data, x.data,
				   DCP_AES128_BLOCK_SIZE);
	if (ret)
		goto out;

	memcpy(output, x.data, DCP_AES128_BLOCK_SIZE);
				...
				const uint8_t *src, uint8_t *dst, size_t size)
{
	...
	desc->src_buffer = (uint8_t *)input.paddr;
	desc->dest_buffer = (uint8_t *)output.paddr;
	desc->payload = (uint8_t *)data->payload.paddr;
	desc->buff_size = size;

	cache_operation(TEE_CACHECLEAN, data->payload.data, data->payload_size);
	cache_operation(TEE_CACHECLEAN, input.data, size);
	cache_operation(TEE_CACHECLEAN, output.data, size);
			     ...
			     const uint8_t *data, size_t len)
{
	...
	nb_blocks = (hashdata->ctx_size + len) / DCP_SHA_BLOCK_SIZE;
			    ...
			    size_t digest_size)
{
	...
	if ((desc->ctrl0 & DCP_CONTROL0_HASH_INIT) && hashdata->ctx_size == 0) {
		...
		desc->src_buffer = (uint8_t *)hashdata->ctx.paddr;
		desc->buff_size = hashdata->ctx_size;
		desc->payload = (uint8_t *)payload.paddr;

		cache_operation(TEE_CACHECLEAN, hashdata->ctx.data,
				hashdata->ctx_size);
		cache_operation(TEE_CACHEFLUSH, payload.data, payload_size);
		...
		dcp_copy_backwards(payload.data, digest, payload_size);
