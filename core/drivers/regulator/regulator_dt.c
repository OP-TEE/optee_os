				     ...
				     uint32_t *supply_phandle)
{
	...
	if (prop_len > sizeof(supply_prop_stack)) {
		supply_prop = calloc(1, prop_len);
		if (!supply_prop)
			return TEE_ERROR_OUT_OF_MEMORY;
	} else {
		supply_prop = supply_prop_stack;
	}
	...
	if (supply_prop != supply_prop_stack)
		free(supply_prop);
...
static TEE_Result release_regulator_pending_lists(void)
{
	...
	if (!SLIST_EMPTY(&pending_regu_list))
		DMSG("Some regualtors were not supplied:");
