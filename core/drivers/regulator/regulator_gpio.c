...
static TEE_Result regulator_gpio_list_voltages(struct regulator *regulator,
					       int **levels_uv, size_t *count)
{
	...
	if (count)
		*count = 2;
	if (levels_uv)
		*levels_uv = regu->voltage_levels_uv;
...
static const struct regulator_ops regulator_gpio_ops = {
	...
	.supported_voltages = regulator_gpio_list_voltages,
};
					 ...
					 struct regulator_gpio *regu)
{
	...
	/*
	 * DT bindings allows more than 1 GPIO to controller more than
	...
	if (dt_driver_device_from_node_idx_prop("gpios", fdt, node, 1,
		...
		EMSG("Multiple GPIOs not supported for regulator level control");
	...
	if (!cuint || len != 4 * sizeof(fdt32_t)) {
		EMSG("Regulator node %s expects 2 levels from property \"states\"",
