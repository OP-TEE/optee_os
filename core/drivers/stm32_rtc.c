...
static TEE_Result stm32_rtc_init(void)
{
	...
	if (IS_ENABLED(CFG_STM32_RTC_HIGH_ACCURACY)) {
		...
		pred_s = (pred_s << RTC_PRER_PREDIV_S_SHIFT) &
			 RTC_PRER_PREDIV_S_MASK;
		pred_a = (pred_a << RTC_PRER_PREDIV_A_SHIFT) &
			 RTC_PRER_PREDIV_A_MASK;
	}

	/* Return if there is nothing to initialize */
	if (cr & RTC_CR_FMT ||
	    (IS_ENABLED(CFG_STM32_RTC_HIGH_ACCURACY) &&
	    prer == (pred_s | pred_a)))
		return TEE_SUCCESS;
	...
	if (res) {
		EMSG("Can't enter in init mode. Fail to initialize RTC.");
	...
	if (res) {
		EMSG("Can't sync RTC. Fail to initialize RTC.");
		stm32_rtc_write_protect();
		return TEE_ERROR_BUSY;
	}
					     ...
					     struct optee_rtc_time *tm)
{
	...
	/*
	 * If the next month has less days than the current month,
	 * then the alarm day must be less than or equal to the maximum
	 * number of days in the next month.
	 * If we are January 31st, and the next month is February,
	 * then the alarm day must be less than or equal to 28 or 29.
	 */
	if (tm->tm_mday < max->tm_mday)
		if (tm->tm_mday > rtc_get_month_days(max->tm_mon, max->tm_year))
			return TEE_ERROR_BAD_PARAMETERS;
				  ...
				  const void *compat_data)
{
	...
	if (IS_ENABLED(CFG_DRIVERS_RTC))
		rtc_register(&stm32_rtc);

	if (IS_ENABLED(CFG_CORE_ASYNC_NOTIF) && rtc_dev.is_secured) {
