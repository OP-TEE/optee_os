...
static TEE_Result versal_mbox_write_req(struct ipi_cmd *cmd)
{
	...
	for (i = 0; i < MAX_IPI_BUF; i++) {
		...
		if (!IS_ALIGNED((uintptr_t)cmd->ibuf[i].mem.buf,
				CACHELINE_LEN)) {
			EMSG("address not aligned: buffer %ld - 0x%p", i,
			     cmd->ibuf[i].mem.buf);
			return TEE_ERROR_GENERIC;
		}

		if (!IS_ALIGNED(cmd->ibuf[i].mem.alloc_len, CACHELINE_LEN)) {
			EMSG("length not aligned: buffer %ld - %ld",
				       ...
				       uint32_t *status)
{
	...
	*status = *(uint32_t *)ipi.rsp;
	...
	for (i = 0; i < MAX_IPI_BUF; i++) {
		...
		if (!IS_ALIGNED((uintptr_t)cmd->ibuf[i].mem.buf,
				CACHELINE_LEN)) {
			EMSG("address not aligned: buffer %ld - 0x%p",
			      ...
			      uint32_t *err)
{
	...
	if (remote_status) {
		if (err)
			*err = remote_status;
		/*
		 * Check the remote code (FSBL) xplmi_status.h and the relevant
		 * service error (ie, xsecure_error.h)
		 */
		DMSG("PLM: plm status = 0x%x, lib_status = 0x%x",
