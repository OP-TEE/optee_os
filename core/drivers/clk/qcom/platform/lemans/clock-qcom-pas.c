...
/*
 * Set up clocks for the LPASS / ADSP QDSP6, mirroring Setup_LPASSProcessor in
 * the reference ClockPIL driver. Unlike the Turing NSP, the Q6 PLL and core RCG
 * are configured here (before the boot FSM is triggered in lpass_fw_start). The
 * LPASS subsystem window is already mapped by the PAS PTA mem-setup, so each
 * sub-block base is resolved via io_pa_or_va against LPASS_BASE + offset.
 */
...
TEE_Result qcom_clock_enable_pas_processor(enum qcom_clk_group group)
{
	...
	case QCOM_CLKS_LPASS:
		/*
		 * LPASS configures its Q6 PLL and switches the core
		 * RCG during the setup stage (lpass_setup, before the
...
TEE_Result qcom_clock_pas_reset(enum qcom_clk_group group)
{
	...
	case QCOM_CLKS_LPASS:
		 ...
		 * the SSR / tear-down path; cold bring-up brings the Q6
		 * up from a known state and the LPASS core HM domain is
		 * already powered at TZ boot.
		 */
		return TEE_SUCCESS;
	default:
		return TEE_ERROR_NOT_SUPPORTED;
