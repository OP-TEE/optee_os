...
static void trng_clrset32(vaddr_t addr, size_t off, uint32_t mask,
			  uint32_t val)
{
				      ...
				      size_t len)
{
	...
	for (bcnt = 0; bcnt < bursts; bcnt++) {
		...
		if ((val & TRNG_STATUS_DTF_MASK) == TRNG_STATUS_DTF_MASK) {
...
uint8_t hw_get_random_byte(void)
{
	...
	if (trng_generate(&trng, random, TRNG_SEC_STRENGTH_LEN, false))
		panic();

	return random[TRNG_SEC_STRENGTH_LEN / 2];
}
...
{
	uint8_t pers_str[TRNG_PERS_STR_LEN] = {
