...
static enum CAAM_Status do_check_data(void)
{
	...
	default:
		...
		do {
			...
			ret = caam_jr_dequeue(wait_jobs, 100);
		} while (loop--);
...
static TEE_Result do_rng_read(uint8_t *buf, size_t len)
{
	...
	do {
		...
		rng = &rng_privdata->databuf[rng_privdata->dataidx];
		RNG_TRACE("Context #%d contains %zu data asked %zu (%zu)",
			      ...
			      uint32_t *desc)
{
	...
	uint8_t sh_idx = 0;
	uint8_t nb_max_sh = nb_sh;

	/* Read the SH and secure key status */
	key_loaded = caam_hal_rng_key_loaded(rng_privdata->baseaddr);
	RNG_TRACE("RNG SH Status 0x%08" PRIx32 " - Key Status %d", sh_status,
		  key_loaded);

	while (sh_status & (1 << sh_idx))
		sh_idx++;

	RNG_TRACE("Instantiation start at SH%d (%d)", sh_idx, nb_max_sh);
...
enum CAAM_Status caam_rng_instantiation(void)
{
	 ...
	 *    |----------------------|
	 */
	desc = caam_alloc_desc(1 + nb_sh + ((nb_sh - 1) * 3) + 4 + 1);
	if (!desc) {
...
/*
 * Initialize the RNG module and do the instantation of the
...
#ifdef CFG_CRYPTO_RNG_HW
/*
 * Crypto interface used in HW RNG supported
 * Fills input buffer buf with blen random bytes
 *
 * @blen  Number of random bytes to read
...
/*
 * This function is need by the core/crypto/rng_hw.c file
