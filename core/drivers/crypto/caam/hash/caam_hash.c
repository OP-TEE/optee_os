			    ...
			    size_t len)
{
	...
	HASH_TRACE("Update Type 0x%X - Input @0x%p-%zu", alg->type,
		   data, inlen);
			   ...
			   size_t len)
{
	...
	HASH_TRACE("Final Type 0x%X - Digest @0x%p-%zu", alg->type,
				     ...
				     uint8_t hash_id)
{
	...
	unsigned int algo = hash_id - TEE_MAIN_ALGO_MD5;

	HASH_TRACE("Allocate Context (0x%p) algo %d", ctx, algo);
