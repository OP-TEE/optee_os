 ...
 * CAAM can manage 32 or 64 bits address function of its version and the
 * device.
 */
/*
				    ...
				    uint8_t nb_jobs)
{
	...
	jr_priv->inrings =
		caam_alloc_align(nb_jobs * sizeof(struct inring_entry));
	jr_priv->outrings =
		caam_alloc_align(nb_jobs * sizeof(struct outring_entry));

	/* Allocate the callers information */
	jr_priv->callers = caam_alloc(nb_jobs * sizeof(struct caller_info));
...
/*
 * Returns all jobs completed depending on the input @waitJobIds mask.
 *
 ...
 * (@waitJobIds parameter)
 *
 * @waitJobIds  Expected Jobs to be complete
 */
static uint32_t do_jr_dequeue(uint32_t waitJobIds)
{
	...
	do {
		...
		for ((idx_jr = 0), (found = (-1));
		     (idx_jr < jr_privdata->nb_jobs) && (found < 0); idx_jr++) {
			/*
		 ...
		 * is a circular buffer of nb_jobs size.
		 */
				      ...
				      uint32_t *job_id)
{
	...
	for ((idx_jr = 0), (found = false);
	     (idx_jr < jr_privdata->nb_jobs) && (!found); idx_jr++) {
	...
	caam_write_val(cur_inrings, caller->pdesc);

	/* Ensure that physical memory is up to date */
	cache_operation(TEE_CACHECLEAN, (void *)cur_inrings,
...
enum CAAM_Status caam_jr_dequeue(uint32_t job_ids, uint32_t timeout_ms)
{
	...
	do {
		...
		jobComplete = do_jr_dequeue(job_ids);

		if (jobComplete & job_ids)
			return CAAM_NO_ERROR;

		/* Check if JR interrupt otherwise wait a bit */
		if (!caam_hal_jr_poolack_itr(jr_privdata->baseaddr))
			caam_udelay(10);
...
enum CAAM_Status caam_jr_init(struct caam_jrcfg *jrcfg)
{
	...
	jr_privdata->paddr_inrings =
		(uint64_t)virt_to_phys(jr_privdata->inrings);
	jr_privdata->paddr_outrings =
		(uint64_t)virt_to_phys(jr_privdata->outrings);
