...
static void search_smallprime(size_t size, struct caambuf *prime)
{
	...
	for (idx = 0; idx < nbElem; idx++) {
		...
		if (psize == size) {
			...
			RSA_TRACE("Found prime idx %d", idx);
		...
		const struct caambuf *small_prime, const paddr_t desc_prime)
{
	 ...
	 */
	desc_add_word(desc, FIFO_LD(CLASS_1, PKHA_A2,
				NOACTION, data->p->length));
		...
		const paddr_t desc_next)
{
	...
	desc_add_word(desc, HALT_USER(ALL_COND_TRUE, MATH_N, RSA_TRY_FAIL));
	...
	desc_add_word(desc, FIFO_LD(CLASS_1, PKHA_A,
		    NOACTION, data->e->length));
	...
	desclen = desc_get_len(desc);
	if (desclen > GEN_RSA_DESC_ENTRIES)	{
		RSA_TRACE("Descriptor Size too short (%d vs %d)",
					desclen, GEN_RSA_DESC_ENTRIES);
		panic();
	}
		...
		const paddr_t desc_new_q)
{
	...
	/* Next 36 bits are 0 */
	desc_add_word(desc, JUMP_CNO_LOCAL(ALL_COND_TRUE,
					JMP_COND(MATH_Z), 10));
	/* Exit status GOOD Q */
...
enum CAAM_Status caam_prime_gen(struct caam_prime_data *data)
{
	...
	RSA_TRACE("Do prime of %d bytes for security length=%d bits (ERA=%d)",
	...
	if (data->q) {
		...
		paddr_desc_q = paddr_desc_p +
					DESC_SZBYTES(GEN_RSA_DESC_ENTRIES);
	...
	if ((data->q) && (retstatus == CAAM_JOB_STATUS)) {
		 ...
		 */
		RSA_TRACE("Check Prime Q Status 0x%08"PRIx32"",
				jobctx.status);
		...
		if (JRSTA_GET_HALT_USER(jobctx.status) == STATUS_GOOD_Q) {
			...
			RSA_DUMPBUF("Prime P", data->p->data,
					data->p->length);
			RSA_DUMPBUF("Prime Q", data->q->data,
					data->q->length);
