...
static enum hisi_drv_status qm_wait_mb_ready(struct hisi_qm *qm)
{
	...
	return readl_poll_timeout(qm->io_base + QM_MAILBOX_BASE, val,
					      ...
					      uint32_t num)
{
	...
	ret = readl_poll_timeout(qm->io_base + QM_VFT_CFG_RDY, val,
	...
	return readl_poll_timeout(qm->io_base + QM_VFT_CFG_RDY, val,
					  val & QM_FVT_CFG_RDY_BIT, POLL_PERIOD,
					  POLL_TIMEOUT);
}
					   ...
					   uint32_t base, uint32_t num)
{
	...
	for (i = QM_SQC_VFT; i <= QM_CQC_VFT; i++) {
		...
		if (ret) {
			EMSG("QM set type%"PRId32" fail", i);
			return ret;
		}
	}
...
static void qp_memory_uninit(struct hisi_qm *qm, uint32_t id)
{
...
static void qm_cache_writeback(struct hisi_qm *qm)
{
	...
	if (readl_poll_timeout(qm->io_base + QM_CACHE_WB_DONE, val,
			       val & QM_FVT_CFG_RDY_BIT, POLL_PERIOD,
			       POLL_TIMEOUT))
...
static enum hisi_drv_status qm_hw_mem_reset(struct hisi_qm *qm)
{
	...
	return readl_poll_timeout(qm->io_base + QM_MEM_INIT_DONE, val,
					  val & QM_FVT_CFG_RDY_BIT, POLL_PERIOD,
					  POLL_TIMEOUT);
}
...
static enum hisi_drv_status qm_sqc_cfg(struct hisi_qp *qp)
{
	...
	memzero_explicit(sqc, sizeof(struct qm_sqc));
...
static enum hisi_drv_status qm_cqc_cfg(struct hisi_qp *qp)
{
	...
	memzero_explicit(cqc, sizeof(struct qm_cqc));
...
struct hisi_qp *hisi_qm_create_qp(struct hisi_qm *qm, uint8_t sq_type)
{
	...
	memzero_explicit(qp->cqe, sizeof(struct qm_cqe) * HISI_QM_Q_DEPTH);
	qp->sq_type = sq_type;
	qp->sq_tail = 0;
	qp->cq_head = 0;
	qp->cqc_phase = true;
...
enum hisi_drv_status hisi_qp_send(struct hisi_qp *qp, void *msg)
{
	...
	memzero_explicit(sqe, qm->sqe_size);
