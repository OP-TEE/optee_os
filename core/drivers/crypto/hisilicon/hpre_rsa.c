...
static int32_t hpre_rsa_get_random_data(void *prng __unused,
					uint8_t *buf, size_t len)
{
	if (hw_get_random_bytes(buf, len)) {
		EMSG("Fail to get random data");
...
static void hpre_rsa_fill_p_q(struct rsa_keypair *key, mbedtls_mpi *p,
			      mbedtls_mpi *q)
{
	crypto_bignum_copy(key->p, (struct bignum *)p);
	crypto_bignum_copy(key->q, (struct bignum *)q);
}
...
static int32_t hpre_rsa_gen_p_q(size_t nbits, struct rsa_keypair *key)
{
	...
	if (nbits > PRIME_QUALITY_FLAG)
		prime_quality = MBEDTLS_MPI_GEN_PRIME_FLAG_LOW_ERR;

	mbedtls_mpi_write_binary((mbedtls_mpi *)key->e, (uint8_t *)&e,
				 sizeof(uint32_t));
	e = TEE_U32_FROM_BIG_ENDIAN(e);
	mbedtls_mpi_lset(&rsa.E, (int32_t)e);

	while (true) {
		MBEDTLS_MPI_CHK(mbedtls_mpi_gen_prime(&rsa.P, nbits >> 1,
						      prime_quality,
						      hpre_rsa_get_random_data,
						      NULL));

		MBEDTLS_MPI_CHK(mbedtls_mpi_gen_prime(&rsa.Q, nbits >> 1,
						      prime_quality,
						      hpre_rsa_get_random_data,
						      NULL));

		/* Temporarily replace P,Q by P-1, Q-1 */
		MBEDTLS_MPI_CHK(mbedtls_mpi_sub_int(&rsa.P, &rsa.P, 1));
		MBEDTLS_MPI_CHK(mbedtls_mpi_sub_int(&rsa.Q, &rsa.Q, 1));
		MBEDTLS_MPI_CHK(mbedtls_mpi_mul_mpi(&H, &rsa.P, &rsa.Q));

		/* check GCD( E, (P-1)*(Q-1) ) == 1  */
		MBEDTLS_MPI_CHK(mbedtls_mpi_gcd(&G, &rsa.E, &H));
		if (mbedtls_mpi_cmp_int(&G, 1))
			continue;

		MBEDTLS_MPI_CHK(mbedtls_mpi_gcd(&G, &rsa.P, &rsa.Q));
		MBEDTLS_MPI_CHK(mbedtls_mpi_div_mpi(&L, NULL, &H, &G));
		MBEDTLS_MPI_CHK(mbedtls_mpi_inv_mod(&rsa.D, &rsa.E, &L));
	...
	/* Restore P,Q */
	MBEDTLS_MPI_CHK(mbedtls_mpi_add_int(&rsa.P, &rsa.P, 1));
	MBEDTLS_MPI_CHK(mbedtls_mpi_add_int(&rsa.Q, &rsa.Q, 1));

	hpre_rsa_fill_p_q(key, &rsa.P, &rsa.Q);
...
static void hpre_rsa_free_keypair(struct rsa_keypair *key)
{
	if (key) {
...
static TEE_Result hpre_rsa_allocate_keypair(struct rsa_keypair *key,
					    size_t size_bits)
{
	if (!key || !size_bits) {
	...
	key->p = crypto_bignum_allocate(size_bits >> 1);
	if (!key->p)
		goto alloc_fail;

	key->q = crypto_bignum_allocate(size_bits >> 1);
	if (!key->q)
		goto alloc_fail;

	key->dp = crypto_bignum_allocate(size_bits >> 1);
	if (!key->dp)
		goto alloc_fail;

	key->dq = crypto_bignum_allocate(size_bits >> 1);
	if (!key->dq)
		goto alloc_fail;

	key->qp = crypto_bignum_allocate(size_bits >> 1);
	if (!key->qp)
		goto alloc_fail;
...
alloc_fail:
	EMSG("HPRE rsa alloc key pair fail");
	hpre_rsa_free_keypair(key);
...
static TEE_Result hpre_rsa_get_keypair_result(struct hpre_rsa_msg *msg,
					      struct rsa_keypair *key)
{
	...
	d = msg->out;
	n = msg->out + msg->key_bytes;
	qp = n + msg->key_bytes;
	dq = qp + (msg->key_bytes >> 1);
	dp = dq + (msg->key_bytes >> 1);

	ret = crypto_bignum_bin2bn(d, msg->key_bytes, key->d);
	if (ret) {
		EMSG("Fail to bin2bn param d");
		return ret;
	}

	ret = crypto_bignum_bin2bn(n, msg->key_bytes, key->n);
	if (ret) {
		EMSG("Fail to bin2bn param n");
		return ret;
	}

	ret = crypto_bignum_bin2bn(qp, msg->key_bytes >> 1, key->qp);
	if (ret) {
		EMSG("Fail to bin2bn param qp");
		return ret;
	}

	ret = crypto_bignum_bin2bn(dq, msg->key_bytes >> 1, key->dq);
	if (ret) {
		EMSG("Fail to bin2bn param dq");
		return ret;
	}

	ret = crypto_bignum_bin2bn(dp, msg->key_bytes >> 1, key->dp);
	if (ret)
		EMSG("Fail to bin2bn param dp");
...
static void hpre_rsa_params_free(struct hpre_rsa_msg *msg)
{
	...
	case HPRE_ALG_NC_CRT:
		if (msg->is_private) {
			len = HPRE_RSA_CRT_KEY_BUF_SIZE(msg->key_bytes);
			free_wipe(msg->prikey);
		}
		break;
	default:
...
static enum hisi_drv_status hpre_rsa_keypair_alloc(struct hpre_rsa_msg *msg)
{
	uint32_t size = HPRE_RSA_GEN_TOTAL_BUF_SIZE(msg->key_bytes);
	uint8_t *data = NULL;

	data = calloc(1, size);
	if (!data) {
		EMSG("Fail to alloc rsa gen buf");
		return HISI_QM_DRVCRYPT_ENOMEM;
	}

	msg->in = data;
	msg->in_dma = virt_to_phys(msg->in);

	msg->out = data + (msg->key_bytes << 1);
	msg->out_dma = msg->in_dma + (msg->key_bytes << 1);

	return HISI_QM_DRVCRYPT_NO_ERR;
}

static enum hisi_drv_status hpre_rsa_keypair_bn2bin(struct hpre_rsa_msg *msg,
						    struct rsa_keypair *key)
{
	uint32_t e_len = 0;
	uint32_t p_len = 0;
	...
	p_len = crypto_bignum_num_bytes(key->p);
	q_len = crypto_bignum_num_bytes(key->q);

	ret = hpre_bin_from_crypto_bin(e, e, msg->key_bytes, e_len);
	if (ret) {
		EMSG("Fail to transfer rsa e from crypto_bin to hpre_bin");
		return ret;
	}

	ret = hpre_bin_from_crypto_bin(p, p, msg->key_bytes >> 1, p_len);
	if (ret) {
		EMSG("Fail to transfer rsa p from crypto_bin to hpre_bin");
		return ret;
	}

	ret = hpre_bin_from_crypto_bin(q, q, msg->key_bytes >> 1, q_len);
	if (ret)
		EMSG("Fail to transfer rsa q from crypto_bin to hpre_bin");

	return ret;
}

static TEE_Result hpre_rsa_keypair_init(struct hpre_rsa_msg *msg,
					struct rsa_keypair *key,
					size_t key_bits)
{
	enum hisi_drv_status ret = HISI_QM_DRVCRYPT_NO_ERR;

	msg->alg_type = HPRE_ALG_KG_CRT;
	msg->key_bytes = hpre_rsa_get_hw_kbytes(key_bits);
	if (!msg->key_bytes)
		return TEE_ERROR_BAD_PARAMETERS;

	ret = hpre_rsa_keypair_alloc(msg);
	if (ret)
		return TEE_ERROR_BAD_STATE;

	ret = hpre_rsa_keypair_bn2bin(msg, key);
	if (ret) {
...
static TEE_Result hpre_rsa_gen_keypair(struct rsa_keypair *key, size_t key_bits)
{
	struct hpre_rsa_msg msg = { };
	TEE_Result ret = TEE_SUCCESS;

	if (!key || !key->n || !key->e || !key->d || !key->p || !key->q ||
	    !key->dp || !key->dq || !key->qp) {
		EMSG("Invalid rsa gen key pair input parameter");
		return TEE_ERROR_BAD_PARAMETERS;
	}

	if (!crypto_bignum_num_bits(key->e)) {
		EMSG("Invalid parameter e");
		return TEE_ERROR_BAD_PARAMETERS;
	}

	if (!crypto_bignum_num_bits(key->p) ||
	    !crypto_bignum_num_bits(key->q)) {
		if (hpre_rsa_gen_p_q(key_bits, key)) {
			EMSG("Fail to gen prime p and q");
			return TEE_ERROR_BAD_PARAMETERS;
		}
	}

	ret = hpre_rsa_keypair_init(&msg, key, key_bits);
	if (ret) {
	...
	ret = hpre_rsa_get_keypair_result(&msg, key);
...
static enum hisi_drv_status hpre_rsa_encrypt_alloc(struct hpre_rsa_msg *msg)
{
	...
	msg->in_dma = msg->pubkey_dma + (msg->key_bytes << 1);
...
static enum hisi_drv_status hpre_rsa_crt_decrypt_alloc(struct hpre_rsa_msg *msg)
{
	...
	if (!msg->prikey_dma) {
		EMSG("Fail to get prikey dma addr");
		free(data);
		return HISI_QM_DRVCRYPT_EFAULT;
	}

	msg->in = data + (msg->key_bytes << 1) + (msg->key_bytes >> 1);
	msg->in_dma = msg->prikey_dma + (msg->key_bytes << 1) +
		      (msg->key_bytes >> 1);
...
static const struct drvcrypt_rsa driver_rsa = {
	.alloc_keypair = hpre_rsa_allocate_keypair,
	.alloc_publickey = hpre_rsa_allocate_publickey,
	.free_publickey = hpre_rsa_free_publickey,
	.free_keypair = hpre_rsa_free_keypair,
	.gen_keypair = hpre_rsa_gen_keypair,
