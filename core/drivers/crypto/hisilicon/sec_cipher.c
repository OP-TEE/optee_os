...
static void ctr_iv_inc(uint64_t *ctr, uint32_t inc)
{
	...
	if (ADD_OVERFLOW(v1, inc, &v1)) {
		ctr[0] = TEE_U64_FROM_BIG_ENDIAN(ctr[0]) + 1;
		ctr[0] = TEE_U64_TO_BIG_ENDIAN(ctr[0]);
	}
...
static TEE_Result sec_cipher_init(void)
{
	...
	if (ret)
		EMSG("Sec cipher register to crypto fail");
