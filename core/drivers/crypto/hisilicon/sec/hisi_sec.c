...
struct hisi_qp *hisi_sec_create_qp(uint8_t sq_type)
{
	...
	SLIST_FOREACH(cur_dev, &sec_list, link) {
		qm = &cur_dev->qm;
		free_qp_num = (qm->fun_type == HISI_QM_HW_PF ?
			HISI_QM_PF_Q_NUM : HISI_QM_VF_Q_NUM) - qm->qp_in_used;
...
{
	/* HISI_QM_HW_V2 version need to close clock gating */
...
static enum hisi_drv_status sec_engine_init(struct acc_device *sec_dev)
{
	...
	io_setbits32(qm->io_base + SEC_CONTROL_REG, SEC_TRNG_EN_MASK);
...
static enum hisi_drv_status sec_qm_init(struct acc_device *sec_dev)
{
	...
	if (qm->fun_type == HISI_QM_HW_PF) {
		...
		IMSG("SEC hardware version is 0x%" PRIx32, qm->version);
...
static TEE_Result sec_probe(void)
{
	...
	IMSG("SEC driver init start, version %s", SEC_MODULE_VERSION);
