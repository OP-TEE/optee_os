...
static bool is_hw_support(struct drvcrypt_authenc_init *dinit)
{
...
static TEE_Result sec_aead_get_dma(struct authenc_ctx *ae_drv_ctx)
{
	...
	if (!ae_drv_ctx->key_dma) {
		EMSG("Failed to map key_dma");
		return TEE_ERROR_GENERIC;
	}

	ae_drv_ctx->civ_dma = virt_to_phys(ae_drv_ctx->civ);
	if (!ae_drv_ctx->civ_dma) {
		EMSG("Failed to map civ_dma");
		return TEE_ERROR_GENERIC;
	}

	ae_drv_ctx->tag_dma = virt_to_phys(ae_drv_ctx->tag);
	if (!ae_drv_ctx->tag_dma) {
		EMSG("Failed to map tag_dma");
		return TEE_ERROR_GENERIC;
	}
				    ...
				    struct drvcrypt_authenc_init *dinit)
{
	...
	if (ae_drv_ctx->algo == C_ALG_AES && ae_drv_ctx->mode == C_MODE_GCM) {
		ae_drv_ctx->is_hw_support = false;
		ae_soft_ctx = ae_drv_ctx->ae_soft_ctx;
		ret = ae_soft_ctx->ops->init(ae_soft_ctx,
			(TEE_OperationMode)(dinit->encrypt == 0),
...
static TEE_Result sec_authenc_initialize(struct drvcrypt_authenc_init *dinit)
{
	...
	ae_drv_ctx = (struct authenc_ctx *)dinit->ctx;
	hw_support = is_hw_support(dinit);
	if (!hw_support)
...
static void sec_authenc_copy_state(void *dst_ctx, void *src_ctx)
{
	...
	if (!src->is_hw_support) {
		dst->is_hw_support = false;
