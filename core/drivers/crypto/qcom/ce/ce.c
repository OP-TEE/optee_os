				...
				uint8_t *tag, size_t tag_len, bool encrypt)
{
	...
	/* dec_tag bytes fed on DIN are consumed for MAC comparison. */
	...
	/*
	 * DIN/DOUT loop. Every byte clocked in passes through to DOUT, so the
	 * loop always both feeds @din_bytes and drains @dout_bytes; leaving any
	 * DOUT byte unread parks the engine in FINAL_READS and blocks
	 * OPERATION_DONE.
	 */
	timeout = timeout_init_us(CE_POLL_TIMEOUT_US);
