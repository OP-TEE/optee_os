...
/**
 * hwkm_setup_kdf() - Populate a SYSTEM_KDF transaction.
 * @t: Transaction to fill.
 * @dks: Destination key slot.
 * @kdk: Source KDK slot.
 * @mks: Mixing-key slot selector (ignored when bsve->km_mks_en is false).
 ...
 *
 * The HUK is derived once and cached; subsequent calls return the cached value.
 *
 * Derivation sequence:
 *   1. If CFG_HWKM_HUK_MIX_SKDK is enabled, derive an SKDK L3 mixing key from
 *      TZ_SKDK_L2 into the dedicated mixing key scratch slot. L1/L2 static
 *      slots cannot be used directly as MKS (hardware error
 *      HWKM_SYSTEM_KDF_ERR_MKS_L1L2_NOT_ALLOWED), so this one-hop produces a
 *      stable L3 key that carries the SKDK lineage.
 *   2. Derive the UKDK L3 KDK and the final L4 HUK. When
 *      CFG_HWKM_HUK_MIX_SKDK is enabled, the SKDK L3 mixing key is folded into
 *      both steps via BSVE.MKS_EN.
 *
 * Return: TEE_SUCCESS on success, or a TEE_ERROR_* code on failure.
 */
