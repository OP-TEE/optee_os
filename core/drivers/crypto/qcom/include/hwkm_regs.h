...
/* Raw read/write. */
#define HWKM_REG_READ(base, reg)	io_read32((base) + (reg))
#define HWKM_REG_WRITE(base, reg, val)	io_write32((base) + (reg), (val))
...
#define HWKM_TZ_KM_CTL_CRC_CHECK_EN	BIT(HWKM_TZ_KM_CTL_CRC_CHECK_EN_SHIFT)
...
#define HWKM_TZ_KM_STATUS_BIST_ERROR	BIT(HWKM_TZ_KM_STATUS_BIST_ERROR_SHIFT)
...
#define HWKM_TZ_TPKEY_RECEIVE_CTL_EN	BIT(HWKM_TZ_TPKEY_RECEIVE_CTL_EN_SHIFT)
/* Destination key slot that will receive the incoming TPKEY [7:0].
 * Write CRYPTO_DEFAULT_TPKEY here before asserting EN.
 */
#define HWKM_TZ_TPKEY_RECEIVE_CTL_TPKEY_DKS_SHIFT 0U
...
#define HWKM_TZ_TPKEY_RECEIVE_STATUS_TPKEY_DKS_SHIFT 0U
#define HWKM_TZ_TPKEY_RECEIVE_STATUS_TPKEY_DKS	GENMASK_32(7, 0)

/* Resource group - BANKn_CLIENT. */
...
#define HWKM_BANK0_KM_CTL_CMD_ENABLE	BIT(HWKM_BANK0_KM_CTL_CMD_ENABLE_SHIFT)
 ...
 */
#define HWKM_BANK0_KM_STATUS_RSP_FIFO_AVAIL_DATA_SHIFT  0x9U
 ...
 */
#define HWKM_BANK0_KM_STATUS_CMD_FIFO_AVAIL_SPACE_SHIFT 0xeU
 ...
 * Clear pattern: HWKM_REG_WRITE(HWKM_REGS_BASE, HWKM_BANK0_KM_ESR,
 *                           HWKM_REG_READ(HWKM_REGS_BASE + HWKM_BANK0_KM_ESR))
...
/* Write one 32-bit command word per store after polling
 * CMD_FIFO_AVAIL_SPACE.
 */
...
/* Resource group - BANKn_AC_MANAGER. */

/*
 * BANKn_AC_BBAC_m - Bank-Based Access Control (BBAC) bitmaps.
 *
 * Each KM instance exposes one AC register block per security domain ("bank").
 * Within each block, up to five 32-bit BBAC registers cover the full key
 * table in 32-slot windows.
 *
 * Setting a bit to 1 grants the owning security domain ("bank") the right to
 * reference that slot in any command it submits. Clearing a bit causes the
 * hardware to reject any command from that domain that names the slot,
 * returning HWKM_ERR_BBAC_ACCESS in the response error word.
 *
 * BBAC access control is checked only for DKS/SKS (destination/source key
 * slot) fields in the command packet.
 *
 * Example: grant BANK0 (TZ) unrestricted access to all 128 key slots:
 *
 *   HWKM_REG_WRITE(HWKM_REGS_BASE + HWKM_BANK0_AC,
...
/**
 * hwkm_reg_get_field() - Read a register field.
 * @base: HWKM MMIO base.
 ...
 */
static inline uint32_t hwkm_reg_get_field(vaddr_t base, uint32_t reg,
					  uint32_t mask, uint32_t shift)
{
	return (HWKM_REG_READ(base, reg) & mask) >> shift;
}

/**
 * hwkm_reg_set_field() - Update a register field.
 * @base: HWKM MMIO base.
 * @reg: register offset from @base.
 * @mask: field mask.
 * @shift: field shift.
 * @val: new field value, right-justified.
 *
 * Update only the bits covered by @mask.
 */
static inline void hwkm_reg_set_field(vaddr_t base, uint32_t reg,
				      uint32_t mask, uint32_t shift,
				      uint32_t val)
{
	uint32_t regval = HWKM_REG_READ(base, reg);

	regval &= ~mask;
	regval |= (val << shift) & mask;
	HWKM_REG_WRITE(base, reg, regval);
}
