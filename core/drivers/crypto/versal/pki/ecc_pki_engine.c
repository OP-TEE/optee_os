...
/*
 * PKI Engine Completions
 */

#define PKI_PK_STATUS_INTERNALDEBUG		GENMASK_32(3, 0)
#define PKI_PK_STATUS_POINTNOTONCURVE		BIT(4)
#define PKI_PK_STATUS_UNUSED1			BIT(5)
#define PKI_PK_STATUS_OUTOFRANGE		BIT(6)
#define PKI_PK_STATUS_INVALIDMODULUS		BIT(7)
#define PKI_PK_STATUS_NOTIMPLEMENTED		BIT(8)
#define PKI_PK_STATUS_SIGNATURENOTVALID		BIT(9)
#define PKI_PK_STATUS_PARAMNOTVALID		BIT(10)
#define PKI_PK_STATUS_NOTINVERTIBLE		BIT(11)
#define PKI_PK_STATUS_COMPOSITE			BIT(12)
#define PKI_PK_STATUS_NOTQUADRATICRESIDUE	BIT(13)
#define PKI_PK_STATUS_UNUSED2			GENMASK_32(15, 14)
#define PKI_PK_STATUS_ERROR_MASK \
	(PKI_PK_STATUS_POINTNOTONCURVE | PKI_PK_STATUS_OUTOFRANGE | \
	 PKI_PK_STATUS_INVALIDMODULUS | PKI_PK_STATUS_NOTIMPLEMENTED | \
	 PKI_PK_STATUS_SIGNATURENOTVALID | PKI_PK_STATUS_PARAMNOTVALID | \
	 PKI_PK_STATUS_NOTINVERTIBLE | PKI_PK_STATUS_NOTQUADRATICRESIDUE)

#define PKI_CQ_VALUE_SRC		BIT(1)
#define PKI_CQ_VALUE_GEN		BIT(0)
#define PKI_CQ_VALUE_REQID_SHIFT	16
#define PKI_CQ_VALUE_REQID_MASK	\
	GENMASK_32(PKI_CQ_VALUE_REQID_SHIFT + 15, PKI_CQ_VALUE_REQID_SHIFT)

#define PKI_DEFAULT_REQID		UINT16_C(0xB04EU)

#define PKI_EXPECTED_CQ_VALUE \
	(SHIFT_U32(PKI_DEFAULT_REQID, PKI_CQ_VALUE_REQID_SHIFT) | \
	 PKI_CQ_VALUE_GEN)
...
static bool pki_largenum_is_zero_le(const uint8_t *val, size_t len)
{
				 ...
				 size_t *out_sz)
{
	...
	ret = pki_ecc_get_key_size(curve, &bytes, &bits);
	if (ret)
		return ret;
...
static TEE_Result pki_start_operation(uint32_t reqval)
{
	...
	/* Wait for completion */
	for (unsigned int retries = 0; retries < PKI_MAX_RETRY_COUNT;
	     retries++) {
...
static TEE_Result pki_check_status(void)
{
	...
	if (cq_value != PKI_EXPECTED_CQ_VALUE) {
		...
		else if ((cq_value & PKI_CQ_VALUE_REQID_MASK) !=
			 SHIFT_U32(PKI_DEFAULT_REQID, PKI_CQ_VALUE_REQID_SHIFT))
			EMSG("PKI bad completion, unexpected request id 0x%04x",
			     (cq_value & PKI_CQ_VALUE_REQID_MASK) >>
			     PKI_CQ_VALUE_REQID_SHIFT);

		if (cq_value & PKI_CQ_VALUE_SRC)
			EMSG("PKI bad completion, DMA or Scheduler error");

		EMSG("PKI bad completion, CQ VALUE: 0x%08x, STATUS: 0x%04x",
		     cq_value, cq_status);

		return TEE_ERROR_GENERIC;
	}

	if (cq_status & PKI_PK_STATUS_ERROR_MASK) {
		if (cq_status & PKI_PK_STATUS_NOTIMPLEMENTED)
			return TEE_ERROR_NOT_SUPPORTED;
		if (cq_status & PKI_PK_STATUS_SIGNATURENOTVALID)
			return TEE_ERROR_SIGNATURE_INVALID;

		EMSG("PKI Engine error: PK STATUS: 0x%04x", cq_status);

		if (cq_status & (PKI_PK_STATUS_POINTNOTONCURVE |
				 PKI_PK_STATUS_OUTOFRANGE |
				 PKI_PK_STATUS_INVALIDMODULUS |
				 PKI_PK_STATUS_PARAMNOTVALID |
				 PKI_PK_STATUS_NOTINVERTIBLE |
				 PKI_PK_STATUS_NOTQUADRATICRESIDUE))
			     ...
			     const uint8_t *sig, size_t sig_len)
{
	...
	ret = pki_ecc_get_key_size(key->curve, &bytes, &bits);
	if (ret)
		return ret;

	/*
	 * NOTE: Amount of data written into PKI request queue
	 *       (PKI_QUEUE_BUF_SIZE) is - in case of signature verification -
	 *       at most (NIST P521):
	 *
	 *         public key (2x 66 Bytes)
	 *       + signature (2x 66 Bytes)
	 *       + message/hash (SHA2-512: 64 Bytes) + padding (2 Bytes)
	 *       + P521 padding for uint32_t (6 Bytes)
	 *       + 8 descriptor words (8x 4 Bytes)
	 *       = 368 Bytes
	 */

	/* Copy public key */
	pki_crypto_bignum_bn2bin_eswap(key->curve, key->x, addr);
	addr += bytes;
	pki_crypto_bignum_bn2bin_eswap(key->curve, key->y, addr);
	addr += bytes;

	/* Copy signature */
	pki_memcpy_swp(addr, sig, sig_len / 2);
	addr += sig_len / 2;
	pki_memcpy_swp(addr, sig + sig_len / 2, sig_len / 2);
	addr += sig_len / 2;

	/* Copy hash */
	ret = pki_ecc_prepare_msg(algo, msg, msg_len, &len, addr);
	if (ret)
		return ret;
	if (len < bytes)
	...
	if (key->curve == TEE_ECC_CURVE_NIST_P521) {
		memset(addr, 0, PKI_VERIFY_P521_PADD_BYTES);
				     ...
				     uint8_t *sig, size_t *sig_len)
{
	...
	/*
	 * NOTE: Amount of data written into PKI request queue
	 *       (PKI_QUEUE_BUF_SIZE) is - in case of signature generation - at
	 *       most (NIST P521):
	 *
	 *         private key (66 Bytes)
	 *       + ephemeral key (66 Bytes)
	 *       + message/hash (SHA2-512: 64 Bytes) + padding (2 Bytes)
	 *       + P521 padding for uint32_t (2 Bytes)
	 *       + 8 descriptor words (8x 4 Bytes)
	 *       = 232 Bytes
	 */

	/* Copy private key */
	pki_crypto_bignum_bn2bin_eswap(key->curve, key->d, addr);
	addr += bytes;

	/* Copy ephemeral key */
	pki_crypto_bignum_bn2bin_eswap(key->curve, ephemeral->d,
				       (uint8_t *)addr);
	addr += bytes;
					     ...
					     size_t bytes)
{
	...
	while (1) {
		...
		/* rejections sampling */
		if (!pki_largenum_is_zero_le(versal_pki.rq_out, bytes))
			break;
	}
...
TEE_Result versal_ecc_gen_keypair(struct ecc_keypair *s)
{
	...
	ret = pki_ecc_get_key_size(s->curve, &bytes, &bits);
	if (ret)
		return ret;
	...
	ret = versal_ecc_gen_private_key(s->curve, addr, bytes);
	if (ret)
		return ret;
	addr += bytes;

	/* Copy generator point x coordinate */
	memcpy(addr, gx, bytes);
	addr += bytes;

	/* Copy generator point y coordinate */
	memcpy(addr, gy, bytes);
	addr += bytes;
