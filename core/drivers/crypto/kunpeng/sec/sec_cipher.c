...
static int sec_cipher_set_key(struct sec_cipher_ctx *c_ctx, const uint8_t *key1,
	const int key1_len, const uint8_t *key2, const int key2_len);
...
static uint32_t sec_update_iv(struct sec_cipher_ctx *c_ctx)
{
	...
	case C_MODE_CBC:
		...
		if (c_ctx->encrypt == true && c_ctx->len >= c_ctx->iv_len)
			memcpy(c_ctx->iv, c_ctx->out + offset, c_ctx->iv_len);
		if (c_ctx->encrypt == false && c_ctx->len >= c_ctx->iv_len)
...
static int sec_cipher_aes_get_c_key_len(const int key_len, const uint8_t mode,
	uint8_t *c_key_len)
{
	switch (mode) {
	case C_MODE_ECB:
...
TEE_Result sec_cipher_initialize(struct drvcrypt_cipher_init *dinit)
{
	...
	if (dinit == NULL || dinit->ctx == NULL || dinit->key1.data == NULL) {
