...
asu_cmac_sw_free_ctx(struct crypto_mac_ctx *ctx)
{
...
static TEE_Result __maybe_unused
asu_cmac_sw_final(struct crypto_mac_ctx *ctx,
...
static TEE_Result __maybe_unused
asu_cmac_sw_update(struct crypto_mac_ctx *ctx,
		   const uint8_t *data, size_t len)
{
	struct asu_cmac_ctx *cmac_ctx = to_cmac_ctx(ctx);
...
static TEE_Result __maybe_unused
asu_cmac_sw_init(struct crypto_mac_ctx *ctx,
...
#if defined(CFG_AMD_ASU_CMAC)
	ret = drvcrypt_register_cmac(asu_cmac_allocate);
	if (ret) {
		EMSG("ASU CMAC register failed ret=%#" PRIx32, ret);
		goto err;
	}
#endif
