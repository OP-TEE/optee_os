 ...
 *
 * Return: TEE_SUCCESS on success, or TEE_ERROR_BAD_PARAMETERS if the inputs
 * are invalid or the bignum does not fit in the destination buffer.
 */
static TEE_Result asu_rsa_bn2bin_pad(size_t size, struct bignum *from,
				     uint8_t *to)
{
	size_t len = 0;
 ...
 *
 * The allocation size is rounded up to the cache line size so that a cache
 * maintenance operation performed on the payload length never reaches memory
 * outside of this allocation.
 *
 * Return: Aligned buffer on success, or NULL on allocation failure.
 */
static void *asu_rsa_alloc_align_buf(size_t len)
{
			...
			struct asu_rsa_pub_key_comp *key_comp)
{
	...
	ret = asu_rsa_bn2bin_pad(n_size, pub->n, n_bin);
	if (ret)
		return ret;

	ret = asu_rsa_bn2bin_pad(ASU_RSA_MAX_PUB_EXP_LEN, pub->e, e_bin);
	if (ret)
		return ret;
			 ...
			 struct asu_rsa_pvt_key_comp *key_comp)
{
	...
	ret = asu_rsa_bn2bin_pad(n_size, priv->d, d_bin);
	if (ret)
		goto out;
 ...
 *
 * Return: TEE_SUCCESS if exponent is F4, TEE_ERROR_NOT_IMPLEMENTED if the
 * exponent is not F4, or TEE_ERROR_BAD_PARAMETERS if the exponent is NULL.
 */
static TEE_Result asu_rsa_pubexp_is_f4(struct bignum *e)
{
	...
	if (asu_rsa_bn2bin_pad(ASU_RSA_MAX_PUB_EXP_LEN, e, e_bin))
		return TEE_ERROR_NOT_IMPLEMENTED;
					    ...
					    const char *alloc_err)
{
	...
	*buf = asu_rsa_alloc_align_buf(alloc_size);
	if (!*buf) {
		EMSG("%s", alloc_err);
	...
	cache_operation(TEE_CACHEFLUSH, *buf, alloc_size);
					     ...
					     const char *alloc_err)
{
	...
	*buf = asu_rsa_alloc_align_buf(len);
	if (!*buf) {
		EMSG("%s", alloc_err);
		return TEE_ERROR_OUT_OF_MEMORY;
	}

	cache_operation(TEE_CACHEFLUSH, *buf, len);
						 ...
						 uint8_t **label_buf)
{
	...
	*label_buf = asu_rsa_alloc_align_buf(alloc_size);
	if (!*label_buf)
		return TEE_ERROR_OUT_OF_MEMORY;
	...
	cache_operation(TEE_CACHEFLUSH, *label_buf, alloc_size);
...
static TEE_Result asu_rsa_encrypt(struct drvcrypt_rsa_ed *rsa_data)
{
	...
	cache_operation(TEE_CACHEFLUSH, key_comp, sizeof(*key_comp));
				      ...
				      size_t size_bits)
{
	...
	key_obj = asu_rsa_alloc_align_buf(key_obj_req_len);
	if (!key_obj)
		return TEE_ERROR_OUT_OF_MEMORY;

	req.key_metadata.length = size_bytes;
	...
	req.key_metadata.key_attributes = ASU_RSA_KM_KEY_ATTR_PRIME_PRESENT;
	...
	cache_operation(TEE_CACHEFLUSH, key_obj, key_obj_req_len);
	...
	cache_operation(TEE_CACHEINVALIDATE, key_obj, key_obj_req_len);
