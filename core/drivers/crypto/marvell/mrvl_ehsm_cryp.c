...
TEE_Result  mrvl_ehsm_aes_context_store(uint32_t *pcontext_id)
{
	...
	ret = ehsm_context_store(&engine.ehandle, CONTEXT_AES,
				 ehsm_pcontext, NULL);
	if (ret != SEC_NO_ERROR) {
		EMSG("eHSM aes context store failed, ret: %u", ret);
		res = TEE_ERROR_GENERIC;
		goto out;
	} else {
		FMSG("eHSM aes context store passed, ret: %u", ret);
	}
...
TEE_Result  mrvl_ehsm_aes_context_load(uint32_t context_id)
{
	...
	if (ret != SEC_NO_ERROR) {
		EMSG("eHSM aes context load failed, ret: %u", ret);
		res = TEE_ERROR_GENERIC;
	} else {
		FMSG("eHSM aes context load passed, ret: %u", ret);
	}
...
TEE_Result  mrvl_ehsm_aes_context_release(uint32_t context_id)
{
	...
	ret = ehsm_context_release(&engine.ehandle, CONTEXT_AES,
				   context_id, NULL);
	if (ret != SEC_NO_ERROR) {
		EMSG("eHSM aes context free failed, ret: %u", ret);
		res = TEE_ERROR_GENERIC;
	} else {
		FMSG("eHSM aes context free passed, ret: %u", ret);
	}
				  ...
				  bool endian_swap)
{
	...
	if (ret != SEC_NO_ERROR) {
		EMSG("eHSM aes zeroize failed, ret: %u", ret);
		res = TEE_ERROR_GENERIC;
		goto out;
	} else {
		FMSG("eHSM aes zeroize passed, ret: %u", ret);
	}

	ret = ehsm_aes_load_key(&engine.ehandle, key_size * 8, key_data, 0, 0);
	if (ret != SEC_NO_ERROR) {
		EMSG("eHSM load key failed, ret: %u", ret);
		res = TEE_ERROR_GENERIC;
		goto out;
	} else {
		FMSG("eHSM load key passed, ret: %u", ret);
	}
	...
	if (ret != SEC_NO_ERROR) {
		EMSG("eHSM aes gcm init failed, ret: %u", ret);
		res = TEE_ERROR_GENERIC;
		goto out;
	} else {
		FMSG("eHSM aes gcm init passed, ret: %u", ret);
	}
					    ...
					    bool new)
{
	...
	ret = ehsm_aes_process(&engine.ehandle, src, dst, len, 0, new, 0, 0,
			       NULL, NULL);
	if (ret != SEC_NO_ERROR) {
		EMSG("eHSM aes process failed, ret: %u", ret);
		res = TEE_ERROR_GENERIC;
	} else {
		FMSG("eHSM aes process passed, ret: %u", ret);
	}
			      ...
			      uint8_t *iv_data, bool endian_swap)
{
	...
	if (key2_size) {
		...
		if (ret != SEC_NO_ERROR) {
			EMSG("eHSM load key2 failed, ret: %u", ret);
			res = TEE_ERROR_GENERIC;
			goto out;
		} else {
			FMSG("eHSM load key2 passed, ret: %u", ret);
		}
	}
	...
	ret = ehsm_aes_init(&engine.ehandle, !is_dec, key_size * 8, aes_mode, 0,
			    endian_swap);
	if (ret != SEC_NO_ERROR) {
		EMSG("eHSM aes init failed, ret: %u", ret);
		res = TEE_ERROR_GENERIC;
		goto out;
	} else {
		FMSG("eHSM aes init passed, ret: %u", ret);
	}

	ret = ehsm_aes_load_iv(&engine.ehandle, iv_data, endian_swap);
	if (ret != SEC_NO_ERROR) {
		EMSG("eHSM aes load iv failed, ret: %u", ret);
		res = TEE_ERROR_GENERIC;
		goto out;
	} else {
		FMSG("eHSM aes load iv passed, ret: %u", ret);
	}
					...
					bool new, bool final)
{
	...
	ret = ehsm_aes_process(&engine.ehandle, src, dst, len, 0, new, final, 0,
			       NULL, NULL);
	if (ret != SEC_NO_ERROR) {
		EMSG("eHSM aes process update failed, ret: %u", ret);
		res = TEE_ERROR_GENERIC;
	} else {
		FMSG("eHSM aes process update passed, ret: %u", ret);
	}
