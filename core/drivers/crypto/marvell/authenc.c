...
static TEE_Result mrvl_ae_initialize(struct drvcrypt_authenc_init *dinit)
{
	...
	ctx->aad_cur_idx = 0;
	ctx->aad_data = calloc(1, ctx->aad_len);
	if (!ctx->aad_data) {
		ret = TEE_ERROR_OUT_OF_MEMORY;
		goto out_aad_err;
	}
	memset(ctx->aad_data, 0, ctx->aad_len);
...
mrvl_ae_update_payload(struct drvcrypt_authenc_update_payload *dupdate)
{
	...
	struct mrvl_cryp_context *ctx = &c->cryp_ctx;
	size_t iv;
	uint8_t *payload_buf, *encdata_buf;
	size_t payload_buf_len, encdata_buf_len;
	...
	if (ctx->is_new) {
		...
		payload_buf = calloc(1, payload_buf_len);
		if (!payload_buf) {
			ret = TEE_ERROR_OUT_OF_MEMORY;
			goto out;
		}
		...
		encdata_buf = calloc(1, encdata_buf_len);
		if (!encdata_buf) {
			ret = TEE_ERROR_OUT_OF_MEMORY;
		...
		memset(encdata_buf, 0, encdata_buf_len);
		...
		payload_buf = calloc(1, dupdate->src.length);
		if (!payload_buf) {
			ret = TEE_ERROR_OUT_OF_MEMORY;
			goto out;
		}

		encdata_buf = calloc(1, dupdate->dst.length);
		if (!encdata_buf) {
			ret = TEE_ERROR_OUT_OF_MEMORY;
			goto out_enc_buf_err;
		}

		memset(payload_buf, 0, dupdate->src.length);
		memset(encdata_buf, 0, dupdate->dst.length);
...
static TEE_Result mrvl_ae_dec_final(struct drvcrypt_authenc_final *dfinal)
{
	...
	struct mrvl_cryp_context *ctx = &c->cryp_ctx;
	size_t iv;
	uint8_t *encdata_buf, *decdata_buf;
	size_t buf_len;
	...
	if (ctx->is_new) {
		...
		encdata_buf = calloc(1, buf_len);
		if (!encdata_buf) {
			ret = TEE_ERROR_OUT_OF_MEMORY;
			goto out;
		}

		decdata_buf = calloc(1, buf_len);
		if (!decdata_buf) {
			ret = TEE_ERROR_OUT_OF_MEMORY;
			goto out_dec_buf_err;
		}

		memset(encdata_buf, 0, buf_len);
		memset(decdata_buf, 0, buf_len);
