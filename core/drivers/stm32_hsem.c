...
static TEE_Result parse_dt(const void *fdt, int node)
{
	...
	if (!hsem_d->rif_proc_conf)
		panic("Missing memory capacity for HSEM RIF proc configuration");
	...
	if (!hsem_d->conf_data.cid_confs || !hsem_d->conf_data.sec_conf ||
	    !hsem_d->conf_data.priv_conf || !hsem_d->conf_data.access_mask)
		panic("Missing memory capacity for HSEM RIF configuration");
