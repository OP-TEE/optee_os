...
/*
 * Clear the CDSP0 content-protection shared channel before starting the DSP;
 * stale contents block the host from talking to CDSP0. No CDSP1 equivalent.
 */
static TEE_Result cdsp0_reset_shared_channel(void)
{
	...
	/*
	 * The channel is write-back cacheable, so flush the zeros to DDR before
	 * unmapping; otherwise the DSP/fastrpc may read stale cached data.
	 */
