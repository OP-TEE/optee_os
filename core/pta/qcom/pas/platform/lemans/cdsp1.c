...
static const struct fw_rsc_devmem turing_mem_res[] = {
	...
	{ .name = "gcc_turing1_cfg", .flags = IOMMU_READ,
	...
	{ .name = "gcc_gpll7_9", .flags = IOMMU_READ,
	...
	{ .name = "shared_imem", .flags = IOMMU_READ | IOMMU_WRITE,
		.da = 0x146bf000, .pa = 0x146bf000, .len = 0x1000, },
	...
	{ .name = "emac_sgmii", .flags = IOMMU_READ | IOMMU_WRITE,
		.da = 0x2304e000, .pa = 0x2304e000, .len = 0x1000, },
	{ .name = "nsp_7806", .flags = IOMMU_READ,
		.da = 0x90860000, .pa = 0x90860000, .len = 0x20000, },
	/*
	 * SMEM holds the GLINK FIFOs the DSP uses to talk to the apps
	 * processor; without this mapping the DSP faults on its first
	 * SMEM access.
	 */
...
DEFINE_RESOURCE_TABLE(TURING1, ARRAY_SIZE(turing_mem_res));

#define BOOT_FSM_TIMEOUT	10000

static TEE_Result cdsp1_fw_start(struct qcom_pas_data *data)
{
...
static TEE_Result cdsp1_fw_shutdown(struct qcom_pas_data *data)
{
	return qcom_clock_pas_reset(data->clk_group);
}

static TEE_Result cdsp1_get_resource_table(struct resource_table *rt,
					   size_t *rt_size)
{
	...
	static struct resource_table table = {
		.ver = 1,
		.num = TURING1_NUM_MEM_RESOURCES,
		.offset[TURING1_NUM_MEM_RESOURCES - 1] = 0,
	};

	return get_mem_rsc(rt, rt_size, &table, &header,
			   turing_mem_res,
			   TURING1_RESOURCE_TABLE_HEADER_SIZE,
			   TURING1_RESOURCE_TABLE_SIZE);
}

const struct qcom_pas_ops cdsp1_ops = {
	.fw_start = cdsp1_fw_start,
	.fw_shutdown = cdsp1_fw_shutdown,
	.get_resource_table = cdsp1_get_resource_table,
};
