...
static const struct fw_rsc_devmem turing_mem_res[] = {
	...
	{ .name = "gcc_boot_rom", .flags = IOMMU_READ | IOMMU_WRITE,
	...
	{ .name = "gcc_gpdsp1_pll_vote", .flags = IOMMU_READ | IOMMU_WRITE,
	...
	{ .name = "gcc_gpdsp1_cfg", .flags = IOMMU_READ | IOMMU_WRITE,
	...
	{ .name = "mproc_ipc2", .flags = IOMMU_READ | IOMMU_WRITE,
		.da = 0x41f000, .pa = 0x41f000, .len = 0x1000, },
	{ .name = "mproc_ipc3", .flags = IOMMU_READ | IOMMU_WRITE,
		.da = 0x420000, .pa = 0x420000, .len = 0x1000, },
	...
	{ .name = "computel0_ipc2", .flags = IOMMU_READ | IOMMU_WRITE,
		.da = 0x45f000, .pa = 0x45f000, .len = 0x1000, },
	{ .name = "computel0_ipc3", .flags = IOMMU_READ | IOMMU_WRITE,
		.da = 0x460000, .pa = 0x460000, .len = 0x26000, },
	...
	{ .name = "computel1_ipc2", .flags = IOMMU_READ | IOMMU_WRITE,
		.da = 0x49f000, .pa = 0x49f000, .len = 0x1000, },
	{ .name = "computel1_ipc3", .flags = IOMMU_READ | IOMMU_WRITE,
		.da = 0x4a0000, .pa = 0x4a0000, .len = 0x32000, },
	...
	{ .name = "periph_ipc3", .flags = IOMMU_READ | IOMMU_WRITE,
		.da = 0x4e0000, .pa = 0x4e0000, .len = 0x1c000, },
	...
	{ .name = "hwkm_prng_ee", .flags = IOMMU_READ,
		.da = 0x10d9000, .pa = 0x10d9000, .len = 0x1000, },
	...
	{ .name = "qos_misc", .flags = IOMMU_READ | IOMMU_WRITE,
		.da = 0x178e000, .pa = 0x178e000, .len = 0x1000, },
	{ .name = "hw_mutex", .flags = IOMMU_READ | IOMMU_WRITE,
		.da = 0x1f40000, .pa = 0x1f40000, .len = 0x40000, },
	{ .name = "tcsr_regs", .flags = IOMMU_READ | IOMMU_WRITE,
	...
	{ .name = "pdc_gpdsp1", .flags = IOMMU_READ | IOMMU_WRITE,
		.da = 0xb2d0000, .pa = 0xb2d0000, .len = 0x10000, },
	...
	{ .name = "pdc_gpdsp1_b", .flags = IOMMU_READ | IOMMU_WRITE,
		.da = 0xb4d0000, .pa = 0xb4d0000, .len = 0x10000, },
	...
	{ .name = "rpmh_drv_gpdsp", .flags = IOMMU_READ | IOMMU_WRITE,
		.da = 0xc310000, .pa = 0xc310000, .len = 0x1000, },
	...
	/* GP-DSP1 exclusive DDR (RW to GPDSP1 only) */
	{ .name = "gpdsp1_ddr", .flags = IOMMU_READ | IOMMU_WRITE,
		.da = 0x21000000, .pa = 0x21000000, .len = 0x1000000, },
...
DEFINE_RESOURCE_TABLE(GPDSP1, ARRAY_SIZE(turing_mem_res));

#define BOOT_FSM_TIMEOUT	10000

static TEE_Result gpdsp1_fw_start(struct qcom_pas_data *data)
{
...
static TEE_Result gpdsp1_fw_shutdown(struct qcom_pas_data *data)
{
	return qcom_clock_pas_reset(data->clk_group);
}

static TEE_Result gpdsp1_get_resource_table(struct resource_table *rt,
					    size_t *rt_size)
{
	...
	static struct resource_table table = {
		.ver = 1,
		.num = GPDSP1_NUM_MEM_RESOURCES,
		.offset[GPDSP1_NUM_MEM_RESOURCES - 1] = 0,
	};

	return get_mem_rsc(rt, rt_size, &table, &header,
			   turing_mem_res,
			   GPDSP1_RESOURCE_TABLE_HEADER_SIZE,
			   GPDSP1_RESOURCE_TABLE_SIZE);
}

const struct qcom_pas_ops gpdsp1_ops = {
	.fw_start = gpdsp1_fw_start,
	.fw_shutdown = gpdsp1_fw_shutdown,
	.get_resource_table = gpdsp1_get_resource_table,
};
