...
static TEE_Result cdsp_fw_start(struct qcom_pas_data *data)
{
	...
	while (boot_status != Q6SS_BOOT_STATUS_STAGE1) {
	...
	timeout = timeout_init_us(BOOT_TIMEOUT_MS * 1000);
	boot_status = io_read32(qdsp6ss + Q6SS_BOOT_STATUS_REG);
	while (boot_status != Q6SS_BOOT_STATUS_STAGE2) {
		if (timeout_elapsed(timeout))
			break;
		udelay(10);
		boot_status = io_read32(qdsp6ss + Q6SS_BOOT_STATUS_REG);
...
static bool cdsp_wait_power_on(vaddr_t tcsr)
{
	uint64_t timeout = timeout_init_us(1000 * 1000);

	while (!(io_read32(tcsr + TCSR_TURING_PWR_ON) & 0x1)) {
		if (timeout_elapsed(timeout))
			return false;
		udelay(10);
	}
...
static bool cdsp_wait_halt_ack(vaddr_t tcsr)
{
	uint64_t timeout = timeout_init_us(5000 * 1000);

	while (!(io_read32(tcsr + TCSR_TURING_HALTACK) & 0x1)) {
		if (timeout_elapsed(timeout))
			return false;
		udelay(10);
	}
...
static TEE_Result cdsp_fw_shutdown(struct qcom_pas_data *data)
{
	...
	if (cdsp_wait_power_on(tcsr)) {
		cdsp_halt_axi_port(tcsr, true);
		if (!cdsp_wait_halt_ack(tcsr)) {
