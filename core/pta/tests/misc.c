...
static int self_test_asan(void)
{
	...
	if (asan_call_test(&ctx, asan_test_global_stat, "(s) glob overflow") ||
	    asan_call_test(&ctx, asan_test_global, "glob overflow") ||
	    asan_call_test(&ctx, asan_test_global_ro, "glob ro overflow") ||
#ifndef CFG_DYN_CONFIG
	    asan_call_test(&ctx, asan_test_stack, "stack overflow") ||
#endif
	    asan_call_test(&ctx, asan_test_malloc, "malloc") ||
	    asan_call_test(&ctx, asan_test_malloc2, "malloc2") ||
	    asan_call_test(&ctx, asan_test_use_after_free, "use_after_free") ||
	    asan_call_test(&ctx, asan_test_memcpy_dst, "memcpy_dst") ||
	    asan_call_test(&ctx, asan_test_memcpy_src, "memcpy_src") ||
	    asan_call_test(&ctx, asan_test_memset, "memset")) {
		ret = -1;
	}
