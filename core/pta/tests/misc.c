...
static volatile char sgbuf[ASAN_TEST_BUF_SIZE] = {0};
volatile char gbuf[ASAN_TEST_BUF_SIZE] = {0};
			  ...
			  const char __maybe_unused *desc)
{
	...
	if (setjmp(asan_test_jmp) == ASAN_TEST_SUCCESS) {
		ret = 0;
	} else {
		test(ctx);
		ret = -1;
...
{
	volatile char buf[ASAN_TEST_BUF_SIZE] = {0};

	buf[ASAN_TEST_BUF_SIZE] = ctx->write_value;
	(void)buf;
}
#endif
...
{
	sgbuf[ASAN_TEST_BUF_SIZE] = ctx->write_value;
	(void)sgbuf;
}

static void asan_global(struct asan_test_ctx *ctx)
{
	gbuf[ASAN_TEST_BUF_SIZE] = ctx->write_value;
	(void)gbuf;
}
...
static void asan_malloc(struct asan_test_ctx *ctx)
{
	...
	if (ctx->pmalloc1)
		ctx->pmalloc1[ASAN_TEST_BUF_SIZE] = ctx->write_value;
}
...
static void asan_malloc2(struct asan_test_ctx *ctx)
{
	...
	p = ctx->pmalloc2[1];
	p[aligned_size] = ctx->write_value;
	(void)p;
}
...
static void asan_memcpy_dst(struct asan_test_ctx *ctx __maybe_unused)
{
	static char b[ASAN_TEST_BUF_SIZE + 1] = {0};
	static char a[ASAN_TEST_BUF_SIZE] = {0};

	memcpy(a, b, sizeof(b));
	(void)a;
}

static void asan_memcpy_src(struct asan_test_ctx *ctx __maybe_unused)
{
	static char b[ASAN_TEST_BUF_SIZE] = {0};
	static char a[ASAN_TEST_BUF_SIZE + 1] = {0};
...
static void asan_memset(struct asan_test_ctx *ctx)
{
	...
	memset(b, ctx->write_value, ASAN_TEST_BUF_SIZE + 1);
	(void)b;
}
