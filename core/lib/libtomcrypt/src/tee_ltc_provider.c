...
static TEE_Result _cipher_get_ctx_size(uint32_t algo, size_t *size)
{
	switch (algo) {
#if defined(CFG_CRYPTO_AES)
...
TEE_Result _crypto_cipher_alloc_ctx(void **ctx_ret, uint32_t algo)
{
	...
	res = _cipher_get_ctx_size(algo, &ctx_size);
	if (res)
		return res;
...
void _crypto_cipher_free_ctx(void *ctx, uint32_t algo __maybe_unused)
{
	 ...
	 */
	if (ctx)
		assert(!_cipher_get_ctx_size(algo, &ctx_size));
	free(ctx);
}
	...
	const uint8_t *inbuf __maybe_unused, size_t len __maybe_unused)
{
	...
	err = add_entropy(inbuf, len, &prng->state);
	if (err != CRYPT_OK)
		return TEE_ERROR_BAD_STATE;
...
TEE_Result rng_generate(void *buffer, size_t len)
{
	...
	if (!prng->inited) {
		if (start(&prng->state) != CRYPT_OK)
			return TEE_ERROR_BAD_STATE;
		plat_prng_add_jitter_entropy_norpc();
		if (ready(&prng->state) != CRYPT_OK)
			return TEE_ERROR_BAD_STATE;
		prng->inited = true;
	}
	if (read(buffer, len, &prng->state) != len)
		return TEE_ERROR_BAD_STATE;
	return TEE_SUCCESS;
