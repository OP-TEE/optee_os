...
static TEE_Result set_curve_sm2(ecc_key *ltc_key)
{
        const ltc_ecc_curve *curve = NULL;
        int ltc_res = 0;

        ltc_res = ecc_find_curve("SM2", &curve);
        if (ltc_res != CRYPT_OK)
...
static TEE_Result make_ltc_keypair(ecc_key *ltc_key,
				   const struct ecc_keypair *key)
{
					  ...
					  uint8_t *dst, size_t *dst_len)
{
	...
	/* Convert the private key into LTC format */
	res = make_ltc_keypair(&ltc_key, key);
	if (res)
		goto out;
...
static TEE_Result make_ltc_pubkey(ecc_key *ltc_key,
					  ...
					  uint8_t *dst, size_t *dst_len)
{
	...
	/* Convert the private key into LTC format */
	res = make_ltc_pubkey(&ltc_key, key);
	if (res)
		goto out;
