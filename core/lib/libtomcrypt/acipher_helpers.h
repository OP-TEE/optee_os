 ...
 */
static inline TEE_Result __must_check
mp_to_unsigned_bin2(void *num, uint8_t *buf, size_t size)
{
	size_t num_size = mp_unsigned_bin_size(num);
	int ltc_res = CRYPT_OK;

	if (num_size > size)
		return TEE_ERROR_BAD_STATE;

	ltc_res = mp_to_unsigned_bin(num, buf + size - num_size);
