...
static bool is_ht_head_recoverable(const struct tee_fs_htree_image *head)
{
	uint8_t zero_tag[TEE_FS_HTREE_TAG_SIZE] = {0};

	return !memcmp(head->tag, zero_tag, sizeof(head->tag)) &&
	       head->counter == 0;
}
			     ...
			     void *stor_aux, struct tee_fs_htree **ht_ret)
{
	...
	if (create) {
		 ...
		 */
		if (is_ht_head_recoverable(&ht->head)) {
			res = create_and_sync(ht, hash, min_counter);
			if (res != TEE_SUCCESS)
				goto out;
		}
